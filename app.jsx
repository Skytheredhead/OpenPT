...
function describeTopologyChange(before = {}, after = {}) {
  ...
  if (!beforeCount && afterCount)
    return `created topology with ${plural(Object.keys(afterDevices).length, "device")} and ${plural(afterLinks.length, "cable")}`;
  ...
  const onlyDeviceChanges =
    !addedDeviceIds.length && !removedDeviceIds.length && !addedLinkIds.length && !removedLinkIds.length && !changedLinkIds.length;
  if (onlyDeviceChanges && changedDeviceIds.length === 1) {
    ...
    if ((prev.x !== next.x || prev.y !== next.y) && sameJson(withoutPosition(prev), withoutPosition(next)))
      return `moved ${deviceLabel(next)}`;
...
function buildTopologyFromPacketTracer(activity) {
  ...
  for (const src of activity?.devices || []) {
    ...
    const dev = OPT_Engine.makeDevice(
      kind,
      src.name || "PT-Device",
      Number(src.x) || 300,
      Number(src.y) || 240,
...
function defaultServerConfig(device = {}) {
  ...
  return {
    ...
    ftp: {
      service: services.ftp ?? true,
      users: [{ username: "cisco", password: "cisco", permission: "RWDNL" }],
      files: SERVER_FILE_LIBRARY.slice(0, 8),
    },
...
function packetTraceFromRuntimeResult(result, devices, srcId, target, options = {}) {
  return packetTraceFromPlan(
    window.OPT_ProtocolRuntime?.toLegacyPlan?.(result) || result,
...
function mergeProjectIntoTabs(tabs, activeWid, project) {
  ...
  return tabs.map((tab) =>
    tab.id === activeWid ? { ...tab, name: `${title}.opt`, cloudProjectId: project?.id || tab.cloudProjectId } : tab
  );
}
...
function writeLocalProjectRecords(records) {
  try {
    localStorage.setItem(LOCAL_PROJECTS_KEY, JSON.stringify(records.slice(0, 80)));
  } catch (e) {}
}
...
const OPENPT_PRACTICE_LABS = window.OpenPTLabs?.menuItems?.() || [
  {
    key: "etherchannel-vlan",
    title: "VLAN Trunks and EtherChannel",
    desc: "Configure access VLANs, LACP, and a trunking port-channel between two switches.",
  },
  {
    key: "dhcp-routerb",
    title: "RouterB DHCP Pool",
    desc: "Build a DHCP pool with reserved addresses, gateway, DNS, domain, and NetBIOS options.",
  },
...
function makeOpenPtPracticeLab(key) {
  ...
  switch (key) {
    case "etherchannel-vlan":
      return makeEtherchannelVlanLab();
    case "dhcp-routerb":
      return makeDhcpRouterBLab();
    case "ospf-area0-dr":
      return makeOspfArea0DrLab();
    case "ssh-line-access":
      return makeSshLineAccessLab();
    default:
      return null;
  }
}
...
function makeEtherchannelVlanLab() {
  ...
  for (const iface of ["FastEthernet0/1", "FastEthernet0/2"]) {
    ...
    SwitchB.interfaces[iface] = switchPort({
      up: true,
      admUp: true,
      mode: "trunk",
      channelGroup: { id: 1, mode: "active" },
      desc: "to SwitchA",
    });
  }
  SwitchB.interfaces["Port-channel1"] = switchPort({
    up: true,
    admUp: true,
    mode: "trunk",
    trunkEncapsulation: "dot1q",
    desc: "LACP trunk to SwitchA",
  });
  SwitchB.etherchannels = { 1: { protocol: "LACP", members: ["FastEthernet0/1", "FastEthernet0/2"] } };
  for (const [iface, vlan] of [
    ["FastEthernet0/3", 10],
    ["FastEthernet0/4", 10],
    ["FastEthernet0/8", 20],
    ["FastEthernet0/9", 20],
  ...
  const PC1 = OPT_Engine.makeDevice("pc", "PC1", 140, 390, {
    eth0: { ip: "192.168.10.11", mask: "255.255.255.0", gw: "", up: true, admUp: true },
  });
  const PC2 = OPT_Engine.makeDevice("pc", "PC2", 360, 390, {
    eth0: { ip: "192.168.20.11", mask: "255.255.255.0", gw: "", up: true, admUp: true },
  });
  const PC3 = OPT_Engine.makeDevice("pc", "PC3", 640, 390, {
    eth0: { ip: "192.168.10.12", mask: "255.255.255.0", gw: "", up: true, admUp: true },
  });
  const PC4 = OPT_Engine.makeDevice("pc", "PC4", 860, 390, {
    eth0: { ip: "192.168.20.12", mask: "255.255.255.0", gw: "", up: true, admUp: true },
  });
  const links = [
  ...
  const activity = practiceActivity({
    title: "VLAN Trunks and EtherChannel",
    instructionsHtml: practiceLabInstructions(
    ...
    assessmentItems: [
      ...
      { device: "SwitchA", iface: "Port-channel1", name: "switchport mode trunk", components: "Trunk Configuration" },
      {
        device: "SwitchA",
        iface: "Port-channel1",
        name: "switchport trunk encapsulation dot1q",
        components: "Trunk Configuration",
        value: "switchport trunk encapsulation dot1q",
      },
    ],
  });
  return {
    title: activity.title,
    fileName: "vlan-etherchannel",
    devices: { [SwitchA.id]: SwitchA, [SwitchB.id]: SwitchB, [PC1.id]: PC1, [PC2.id]: PC2, [PC3.id]: PC3, [PC4.id]: PC4 },
    links,
    activity,
  };
}
...
function makeDhcpRouterBLab() {
  ...
  const HostD = OPT_Engine.makeDevice("pc", "HostD", 620, 480, {
    eth0: { ip: "192.168.44.10", mask: "255.255.255.0", gw: "192.168.44.1", up: true, admUp: true },
  });
  const links = [
  ...
  const activity = practiceActivity({
    ...
    assessmentItems: [
      {
        device: "RouterB",
        name: "ip dhcp excluded-address 192.168.11.1 192.168.11.10",
        components: "DHCP Configuration",
        value: "ip dhcp excluded-address 192.168.11.1 192.168.11.10",
      },
      { device: "RouterB", name: "ip dhcp pool pool1", components: "DHCP Configuration", value: "ip dhcp pool pool1" },
      {
        device: "RouterB",
        name: "network 192.168.11.0 255.255.255.0",
        components: "DHCP Configuration",
        value: "network 192.168.11.0 255.255.255.0",
      },
      ...
      {
        device: "RouterB",
        name: "netbios-name-server 192.168.11.4",
        components: "DHCP Configuration",
        value: "netbios-name-server 192.168.11.4",
      },
    ],
  });
  return {
    title: activity.title,
    fileName: "routerb-dhcp",
    devices: {
      [RouterA.id]: RouterA,
      [RouterB.id]: RouterB,
      [RouterC.id]: RouterC,
...
function makeOspfArea0DrLab() {
  ...
  const HostA = OPT_Engine.makeDevice("pc", "HostA", 460, 40, {
    eth0: { ip: "192.168.1.10", mask: "255.255.255.0", gw: "192.168.1.1", up: true, admUp: true },
  });
  const HostB = OPT_Engine.makeDevice("pc", "HostB", 220, 540, {
    eth0: { ip: "192.168.2.10", mask: "255.255.255.0", gw: "192.168.2.1", up: true, admUp: true },
  });
  const HostC = OPT_Engine.makeDevice("pc", "HostC", 700, 540, {
    eth0: { ip: "192.168.3.10", mask: "255.255.255.0", gw: "192.168.3.1", up: true, admUp: true },
  });
  const links = [
  ...
  const activity = practiceActivity({
    ...
    assessmentItems: [
      ...
      ]),
      {
        ...
        name: "ip ospf priority 255",
        components: "OSPF Configuration",
        value: "ip ospf priority 255",
      },
      {
        device: "RouterC",
        iface: "FastEthernet0/0",
        name: "ip ospf priority 0",
        components: "OSPF Configuration",
        value: "ip ospf priority 0",
      },
    ],
  });
  return {
    title: activity.title,
    fileName: "ospf-area0-dr",
    devices: {
      [RouterA.id]: RouterA,
      [RouterB.id]: RouterB,
      [RouterC.id]: RouterC,
...
function makeSshLineAccessLab() {
  ...
  const PC1 = OPT_Engine.makeDevice("pc", "PC1", 80, 170, {
    eth0: { ip: "10.10.10.11", mask: "255.255.255.0", gw: "", up: true, admUp: true },
  });
  const PC2 = OPT_Engine.makeDevice("pc", "PC2", 80, 380, {
    eth0: { ip: "10.10.20.11", mask: "255.255.255.0", gw: "", up: true, admUp: true },
  });
  const links = [
  ...
  const activity = practiceActivity({
    ...
    assessmentItems: [
      ...
      {
        device: "CSW1",
        name: "crypto key generate rsa modulus 2048",
        components: "SSH Configuration",
        value: "crypto key generate rsa modulus 2048",
      },
...
function App({ initialViewMode = null, initialHomeAction = null } = {}) {
  ...
  const [viewMode, setViewMode] = useState(() => {
    ...
    try {
      return localStorage.getItem("openpt:viewMode") === "home" ? "app" : localStorage.getItem("openpt:viewMode") || "app";
    } catch (e) {
      return "app";
    }
  });
  ...
  const navigateAppRoute = React.useCallback((to, options = {}) => {
    ...
    else if (isInternalAppRoute(nextPath)) setViewMode("app");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("openpt:viewMode", viewMode);
    } catch (e) {}
  }, [viewMode]);
  const [bottomCollapsed, setBottomCollapsed] = useState(() => {
    try {
      return localStorage.getItem("openpt:bottom-collapsed") === "1";
    } catch (e) {
      return false;
    }
  });
  ...
  const [recentPingTargets, setRecentPingTargets] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("openpt:recent-pings") || "[]");
    } catch (e) {
      return [];
    }
  });
  ...
  const [cliGhostSuggestions, setCliGhostSuggestions] = useState(() => {
    try {
      return localStorage.getItem("openpt:cli-ghost") !== "0";
    } catch (e) {
      return true;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("openpt:server-module-width", String(serverModuleWidth));
    } catch (e) {}
  }, [serverModuleWidth]);
  useEffect(() => {
    try {
      localStorage.setItem("openpt:apps-sidebar-width", String(appsSidebarWidth));
    } catch (e) {}
  }, [appsSidebarWidth]);

  useEffect(() => {
    try {
      localStorage.setItem("openpt:bottom-collapsed", bottomCollapsed ? "1" : "0");
    } catch (e) {}
  }, [bottomCollapsed]);
  useEffect(() => {
    try {
      localStorage.setItem("openpt:cli-ghost", cliGhostSuggestions ? "1" : "0");
    } catch (e) {}
  }, [cliGhostSuggestions]);
  useEffect(() => {
    try {
      localStorage.setItem("openpt:recent-pings", JSON.stringify(recentPingTargets.slice(0, 8)));
    } catch (e) {}
  }, [recentPingTargets]);
  ...
  const loadPracticeLab = async (key, options = {}) => {
    ...
    if (shouldConfirm) {
      const ok = await requestConfirm({
        title: `Load ${lab.title}?`,
        message: "Replace the current topology with this practice lab?",
        confirmLabel: "Load lab",
        danger: true,
      });
      if (!ok) return;
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem("openpt:bottom-height", String(bottomPanelHeight));
    } catch (e) {}
  }, [bottomPanelHeight]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    lessonWorkspaceSaveTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setToast({
          kind: "err",
          msg: err.status === 413 ? "Lesson workspace is too large to save." : err.message || "Could not save lesson workspace",
        });
      }
    }, 900);
  ...
  const resetAllLessonProgress = async () => {
    if (!syncClient || !cloudUser) {
      openLessonSignIn();
      return;
    }
    const ok = await requestConfirm({
      ...
      message:
        "Are you sure? This will clear every completed checkpoint, XP total, roadmap status, and saved lesson workspace for this account.",
    ...
    try {
      ...
      const data = await syncClient.resetAllLessonProgress();
      setLessonDashboard(data.dashboard || null);
      setToast({ kind: "ok", msg: "Learning progress reset." });
      if (learnLessonId) navigateAppRoute(LEARN_URL);
    } catch (err) {
      setToast({ kind: "err", msg: err.message || "Could not reset learning progress" });
    }
  };
  ...
  const resetLessonProgress = async (lessonId) => {
    ...
    const lessonTitle = (lessonCatalog?.lessons || []).find((lesson) => lesson.id === lessonId)?.title || "this lesson";
    const ok = await requestConfirm({
      title: "Reset lesson progress?",
      message: `Are you sure? This will clear checkpoints, XP, and the saved workspace for ${lessonTitle}.`,
    ...
    try {
      if (lessonSession?.lessonId === lessonId) {
        ...
        lessonWorkspaceLastJsonRef.current = "";
        lessonRouteStartKeyRef.current = "";
        if (learnLessonId === lessonId) navigateAppRoute(LEARN_URL);
      }
      const data = await syncClient.resetLessonProgress(lessonId);
      setLessonDashboard(data.dashboard || null);
      setToast({ kind: "ok", msg: "Lesson progress reset." });
    } catch (err) {
      setToast({ kind: "err", msg: err.message || "Could not reset lesson progress" });
    }
  };
  ...
  const restoreGuidedLessonWorkspace = (lesson, workspace, progress = {}, { routeToLab = true, labId = null, fallbackLab = null } = {}) => {
    ...
    const targetId =
      tabs.find((tab) => lessonTabMatch(tab, lesson.id, labId || tabInfo.labId || null))?.id ||
      tabInfo.id ||
    ...
    setTimeout(() => {
      lessonWorkspaceHydratingRef.current = false;
    }, 500);
  ...
  const startGuidedLesson = async (lessonId, options = {}) => {
    ...
    if (
      startResult.workspace?.snapshot &&
      restoreGuidedLessonWorkspace(lesson, startResult.workspace, progress, {
        labId,
  ...
  const completeLessonStep = React.useCallback(
    async (stepId, source = "checkpoint", payload = {}) => {
      ...
      setLessonSession((current) => {
        ...
        const nextStepId = data?.lesson?.currentStepId || lesson?.steps?.find((step) => !completedSteps.includes(step.id))?.id || stepId;
        return {
          ...current,
          completedStepIds: completedSteps,
          stepId: nextStepId,
  ...
  const markLessonPing = (srcId, target, plan) => {
    ...
    const matchesCurrentPing = (currentStep?.checks || []).some(
      (check) => check.type === "ping" && check.source === source && String(check.target) === cleanedTarget
    );
    if (!plan?.ok) {
  ...
  useEffect(() => {
    ...
    const ready = (lesson.steps || []).find(
      (step) =>
        !completed.has(step.id) &&
        OpenPTLearn.stepChecksMet(step, {
  ...
  const applyProjectDocument = (document, project = null) => {
    ...
    setActiveBottom(
      document?.uiState?.activeBottom && document.uiState.activeBottom !== "pka-report" ? document.uiState.activeBottom : "events"
    );
  ...
  const applyLocalProjectRecord = async (record) => {
    ...
    setTabs((items) => {
      const nextTab = {
        id: record.id,
        name: `${record.title || doc.title || "Untitled OpenPT project"}.opt`,
    ...
    setSyncStatus(
      record.cloudProjectId
        ? { state: "readonly", message: "Cloud-backed local copy. Acquire edit lease before saving." }
        : { state: "local", message: "Local only" }
    );
  ...
  const renameLocalProject = (id, title) => {
    ...
    if (!clean) return;
    updateLocalProjectRecords((records) =>
      records.map((item) => {
  ...
  const deleteCloudProject = async (project) => {
    ...
    try {
      ...
      if (project.id === cloudProjectId)
        resetSyncState({
          clearProject: true,
          clearShare: true,
          clearSaveCounters: true,
          status: { state: "local", message: "Cloud project deleted; local copy kept" },
        });
  ...
  const saveCloudNow = async ({ force = false, queueOffline = true } = {}) => {
    ...
    const runSave = async () => {
      ...
      try {
        ...
        await Sync.saveLocalDocument(state.shareToken ? `share:${state.shareToken}` : `project:${state.cloudProjectId}`, data.document, {
        ...
        if (err.status === 409) {
          ...
          setSyncStatus({
            state: "readonly",
            message: err.data?.lease?.clientLabel ? `Editing on ${err.data.lease.clientLabel}` : "Edit lease required",
          });
  ...
  const openCloudProject = async (projectId) => {
    ...
    try {
      ...
      try {
        ...
        setSyncStatus({
          state: "readonly",
          message: err.data?.lease?.clientLabel ? `Read-only: editing on ${err.data.lease.clientLabel}` : "Read-only: lease unavailable",
        });
      }
      setProjectsOpen(false);
    } catch (err) {
  ...
  const acquireCurrentLease = async (takeover = false) => {
    ...
    try {
      ...
      setSyncStatus({
        state: "readonly",
        message: err.data?.lease?.clientLabel ? `Editing on ${err.data.lease.clientLabel}` : err.message,
      });
    }
  };
      ...
      syncClient
        ...
        .then((data) => {
          ...
          resetSyncState({
            ...
            clearShare: true,
            clearSaveCounters: true,
            status: { state: "local", message: "Signed out" },
          });
        })
        .catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    syncClient
      ...
      .then((data) => {
        ...
        setDirtyTabs((m) => ({ ...m, [activeWid]: false }));
        setSyncStatus({
          state: data.project.mode === "edit" ? "readonly" : "readonly",
          message: data.project.mode === "edit" ? "Shared project opened. Acquire edit lease to save." : "Read-only share",
        });
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      syncClient
        .renewLease(cloudProjectId, cloudLease.id)
        .then((data) => {
          setCloudLease(data.lease);
        })
        .catch(() => {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      syncClient
        .acquireShareLease(shareToken, false)
        .then((data) => setCloudLease(data.lease))
        .catch(() => {
          setCloudLease(null);
          setSyncStatus({ state: "readonly", message: "Edit lease expired" });
        });
  ...
  const recordPacketEvent = (trace, deviceSnapshot = devices) => {
    if (!trace) return;
    setPacketEvents((items) => [...items.slice(-299), completePacketTrace(trace, deviceSnapshot)]);
  };
  ...
  const reloadDevice = (id) => {
    ...
    setDevices((m) => {
      ...
      log(
        reloaded.startupConfigState ? "ok" : "warn",
        reloaded.hostname,
        reloaded.startupConfigState ? "reloaded from startup-config" : "reloaded with default configuration"
      );
  ...
  const onApplyToDevice = (devId, cmd) => {
    ...
    setDevices((m) => {
      if (cmd.kind === "host-dhcp") {
        ...
        recordPacketEvent(
          packetTraceFromRuntimeResult(result, m, devId, "DHCP", { kind: "dhcp", protocol: "dhcp", app: { message } }),
          result.devices || m
        );
      ...
      switch (cmd.kind) {
        ...
        case "reload": {
          ...
          log(
            d.startupConfigState ? "ok" : "warn",
            d.hostname,
            d.startupConfigState ? "reloaded from startup-config" : "reloaded with default configuration"
          );
          break;
        }
        case "hostname":
        ...
        case "wireless":
          ...
          d.wireless.ssids = d.wireless.ssids?.length
            ? d.wireless.ssids
            : [
                {
                  name: d.wireless.ssid || "OpenPT",
                  security: d.wireless.security || "open",
                  passphrase: d.wireless.passphrase || "",
                  vlan: d.wireless.vlan || 1,
                  enabled: true,
                },
              ];
          d.wireless.ssids[0] = {
            ...
            security:
              OPT_Engine.normalizeWirelessSecurity?.(
                cmd.field === "security" ? cmd.value : d.wireless.security || d.wireless.ssids[0]?.security || "open"
              ) || (cmd.field === "security" ? cmd.value : d.wireless.security || "open"),
        ...
        case "host-ipv6":
          {
            ...
            ifaces[hostIface] = {
              ...ifaces[hostIface],
              ipv6: normalized,
              ipv6PrefixLength: cmd.prefixLength || 64,
              ipv6Gw: cmd.gw || null,
              ipv6Source: cmd.source || "static",
              ipv6Enabled: true,
              linkLocal: ifaces[hostIface].linkLocal || OPT_Engine.ipv6LinkLocal?.(ifaces[hostIface]),
              up: true,
              admUp: true,
            };
        ...
        case "ipv6-address":
          {
            ...
            ifaces[cmd.iface] = {
              ...ifaces[cmd.iface],
              ipv6: normalized,
              ipv6PrefixLength: cmd.prefixLength,
              ipv6Source: cmd.source,
              ipv6Enabled: !!normalized || ifaces[cmd.iface].ipv6Enabled,
              linkLocal: normalized
                ? ifaces[cmd.iface].linkLocal || OPT_Engine.ipv6LinkLocal?.(ifaces[cmd.iface])
                : ifaces[cmd.iface].linkLocal,
            };
            log(
              normalized ? "ok" : "warn",
              d.hostname,
              normalized
                ? `${ifaceName(cmd.iface)} IPv6 address ${normalized}/${cmd.prefixLength}`
                : `${ifaceName(cmd.iface)} IPv6 address removed`
            );
          }
          break;
        case "ipv6-autoconfig":
          ifaces[cmd.iface] = {
            ...ifaces[cmd.iface],
            ipv6Autoconfig: cmd.value,
            ipv6Source: cmd.value ? "slaac" : ifaces[cmd.iface].ipv6Source,
            ipv6Enabled: cmd.value || ifaces[cmd.iface].ipv6Enabled,
            linkLocal: ifaces[cmd.iface].linkLocal || OPT_Engine.ipv6LinkLocal?.(ifaces[cmd.iface]),
          };
          break;
        case "ipv6-enable":
          ifaces[cmd.iface] = {
            ...ifaces[cmd.iface],
            ipv6Enabled: cmd.value,
            linkLocal: cmd.value
              ? ifaces[cmd.iface].linkLocal || OPT_Engine.ipv6LinkLocal?.(ifaces[cmd.iface])
              : ifaces[cmd.iface].linkLocal,
          };
        ...
        case "routed-port":
          ifaces[cmd.iface] = {
            ...ifaces[cmd.iface],
            routed: cmd.value,
            mode: cmd.value ? undefined : "access",
            vlan: cmd.value ? undefined : ifaces[cmd.iface].vlan || 1,
          };
        ...
        case "port-security":
          ifaces[cmd.iface] = {
            ...ifaces[cmd.iface],
            portSecurity: {
              ...(ifaces[cmd.iface].portSecurity || {}),
              ...cmd,
              enabled: cmd.enabled ?? ifaces[cmd.iface].portSecurity?.enabled ?? true,
            },
          };
        ...
        case "channel-group": {
          ...
          if (!ifaces[po])
            ifaces[po] = {
              ...
              mac: randMac(),
              desc: "",
              mode: ifaces[cmd.iface].mode || "trunk",
              vlan: ifaces[cmd.iface].vlan || 1,
              nativeVlan: ifaces[cmd.iface].nativeVlan || 1,
              allowedVlans: ifaces[cmd.iface].allowedVlans || "all",
            };
          d.etherchannels[cmd.id] = {
            protocol: ["active", "passive"].includes(cmd.mode) ? "LACP" : ["auto", "desirable"].includes(cmd.mode) ? "PAgP" : "static",
            members: [...new Set([...(d.etherchannels[cmd.id]?.members || []), cmd.iface])],
          };
        ...
        case "dhcp-snoop-trust":
          ...
          d.dhcpSnooping.trusted = cmd.value
            ? [...new Set([...(d.dhcpSnooping.trusted || []), cmd.iface])]
            : (d.dhcpSnooping.trusted || []).filter((x) => x !== cmd.iface);
          break;
        case "dai-trust":
          ifaces[cmd.iface] = { ...ifaces[cmd.iface], daiTrust: cmd.value };
          d.dai.trusted = cmd.value
            ? [...new Set([...(d.dai.trusted || []), cmd.iface])]
            : (d.dai.trusted || []).filter((x) => x !== cmd.iface);
          break;
        case "encapsulation":
        ...
        case "service-policy":
          ifaces[cmd.iface] = {
            ...ifaces[cmd.iface],
            servicePolicy: { ...(ifaces[cmd.iface].servicePolicy || {}), [cmd.dir]: cmd.policy },
          };
          break;
        case "pim":
        ...
        case "hsrp":
          ifaces[cmd.iface] = {
            ...ifaces[cmd.iface],
            hsrp: {
              ...(ifaces[cmd.iface].hsrp || {}),
              [cmd.group]: {
                ...(ifaces[cmd.iface].hsrp?.[cmd.group] || {}),
                ...(cmd.ip ? { ip: cmd.ip } : {}),
                ...(cmd.priority ? { priority: cmd.priority } : {}),
                priority: cmd.priority || ifaces[cmd.iface].hsrp?.[cmd.group]?.priority || 100,
              },
            },
          };
          break;
        ...
        case "ip-route":
          d.routes = [
            ...(d.routes || []).filter((r) => !(r.type === "S" && r.dst === cmd.dst && r.mask === cmd.mask && r.via === cmd.via)),
            { dst: cmd.dst, mask: cmd.mask, via: cmd.via, iface: OPT_Engine.ifaceForVia(d, cmd.via), type: "S" },
          ];
        ...
        case "ipv6-route": {
          ...
          d.ipv6Routes = [
            ...(d.ipv6Routes || []).filter(
              (r) => !(r.type === "S" && r.prefix === prefix && r.prefixLength === cmd.prefixLength && r.via === via)
            ),
            {
              prefix,
              prefixLength: cmd.prefixLength,
              via,
              iface: OPT_Engine.ifaceForIpv6Via?.(d, via) || Object.keys(ifaces)[0],
              type: "S",
            },
          ];
        ...
        case "no-ipv6-route": {
          const prefix = OPT_Engine.ipv6NetworkAddress?.(cmd.prefix, cmd.prefixLength) || cmd.prefix;
          const via = OPT_Engine.normalizeIpv6?.(cmd.via) || cmd.via;
          d.ipv6Routes = (d.ipv6Routes || []).filter(
            (r) => !(r.type === "S" && r.prefix === prefix && r.prefixLength === cmd.prefixLength && r.via === via)
          );
        ...
        case "ospf-network": {
          ...
          ospf.networks = [
            ...(ospf.networks || []).filter((n) => !(n.network === cmd.network && n.wildcard === cmd.wildcard && n.area === cmd.area)),
            { network: cmd.network, wildcard: cmd.wildcard, area: cmd.area },
          ];
          d.ospf[cmd.pid] = ospf;
        ...
        case "ospf-network-remove": {
          const ospf = d.ospf[cmd.pid] || { networks: [], passive: [] };
          ospf.networks = (ospf.networks || []).filter(
            (n) => !(n.network === cmd.network && n.wildcard === cmd.wildcard && n.area === cmd.area)
          );
          d.ospf[cmd.pid] = ospf;
        ...
        case "ospf-passive": {
          const ospf = d.ospf[cmd.pid] || { networks: [], passive: [] };
          ospf.passive = cmd.value
            ? [...new Set([...(ospf.passive || []), cmd.iface])]
            : (ospf.passive || []).filter((x) => x !== cmd.iface);
          d.ospf[cmd.pid] = ospf;
          break;
        }
        case "ospf-default":
        ...
        case "routing-network": {
          ...
          db[id].networks = [
            ...(db[id].networks || []).filter((n) => n.network !== cmd.network),
            { network: cmd.network, wildcard: cmd.wildcard, mask: cmd.mask || wildcardToMaskSafe(cmd.wildcard) },
          ];
        ...
        case "routing-passive": {
          const db = cmd.proto === "ospf" ? d.ospf : cmd.proto === "eigrp" ? d.eigrp : cmd.proto === "rip" ? d.rip : d.bgp;
          db[cmd.id] = db[cmd.id] || { networks: [], passive: [], neighbors: [] };
          db[cmd.id].passive = cmd.value
            ? [...new Set([...(db[cmd.id].passive || []), cmd.iface])]
            : (db[cmd.id].passive || []).filter((x) => x !== cmd.iface);
          break;
        }
        case "routing-field": {
        ...
        case "bgp-neighbor":
          ...
          d.bgp[cmd.id].neighbors = [
            ...(d.bgp[cmd.id].neighbors || []).filter((n) => n.ip !== cmd.ip),
            { ip: cmd.ip, remoteAs: cmd.remoteAs },
          ];
          break;
        case "dhcp-pool":
        ...
        case "acl-entry": {
          ...
          d.acls[cmd.name] = d.acls[cmd.name] || { type: cmd.aclType, entries: [] };
          if (cmd.seq != null)
            d.acls[cmd.name].entries = d.acls[cmd.name].entries.filter((existing) => Number(existing.seq) !== Number(cmd.seq));
        ...
        case "route-map-create":
          ...
          if (!d.routeMaps[cmd.name].sequences.some((s) => s.seq === cmd.seq))
            d.routeMaps[cmd.name].sequences.push({ seq: cmd.seq, action: cmd.action });
          break;
        case "route-map-line": {
          d.routeMaps[cmd.name] = d.routeMaps[cmd.name] || { sequences: [] };
          let seq = d.routeMaps[cmd.name].sequences.find((s) => s.seq === cmd.seq);
          if (!seq) {
            seq = { seq: cmd.seq, action: "permit" };
            d.routeMaps[cmd.name].sequences.push(seq);
          }
        ...
        case "aaa-method":
          d.aaa.methods = [
            ...(d.aaa.methods || []).filter((x) => !(x.service === cmd.service && x.list === cmd.list)),
            { service: cmd.service, list: cmd.list, methods: cmd.methods },
          ];
          break;
        case "crypto-rsa":
        ...
        case "policy-map-class":
          ...
          if (!d.qos.policyMaps[cmd.policy].classes.some((c) => c.name === cmd.className))
            d.qos.policyMaps[cmd.policy].classes.push({ name: cmd.className, actions: [] });
          break;
        case "policy-map-action": {
          d.qos.policyMaps[cmd.policy] = d.qos.policyMaps[cmd.policy] || { classes: [] };
          let cls = d.qos.policyMaps[cmd.policy].classes.find((c) => c.name === cmd.className);
          if (!cls) {
            cls = { name: cmd.className, actions: [] };
            d.qos.policyMaps[cmd.policy].classes.push(cls);
          }
  ...
  const animatePath = (plan, snapshot, onDone) => {
    ...
    const linkIdBetween = (fromId, toId) => {
      const link = links.find((item) => (item.a === fromId && item.b === toId) || (item.a === toId && item.b === fromId));
      return link?.id || null;
    };
  ...
  const handlePing = (srcId, target, opts = {}, onComplete) => {
    ...
    if (!opts.silent) {
      (plan.events || [])
        .filter(
          (ev) =>
            ["arp", "nat", "icmp", "stp", "ospf"].includes(ev.proto) &&
            ["request", "reply", "translate", "drop", "echo-request", "echo-reply"].includes(ev.kind)
        )
        .slice(0, 12)
    ...
    if (!opts.silent && opts.record !== false) {
      recordPacketEvent(
        packetTraceFromPlan(plan, plan.devices || devices, srcId, target, {
  ...
  const closeEndpointApp = (tabId) => {
    ...
    const nextActiveApp = remaining
      .slice()
      .reverse()
      .find((item) => item.wid === activeWid && devices[item.deviceId]);
  ...
  const deviceIdByLessonName = React.useCallback(
    (name) => {
  ...
  const lessonNeedsCable = !!(
    activeLessonStep?.checks || []
  ).some((check) => check.type === "topology") && !(lessonSession?.completedStepIds || []).includes(activeLessonStep?.id);
  ...
  const enterLabFromHome = React.useCallback(
    (intent = "lab") => {
  ...
  const lessonMobileSheet =
    lessonSession && activeLesson ? (
        ...
        <div className="lesson-mobile-body">
            ...
            <div className="lesson-mobile-coach">
              ...
              <div className="learn-checks">
                {(activeLessonStep.checks || []).map((check, index) => {
                  const met = OpenPTLearn?.checkMet?.(check, {
                    activity: gradedPtActivity,
                    devices,
                    links,
                    lessonSession,
                    stepId: activeLessonStep.id,
                  });
                  return (
              ...
              <div className="lesson-mobile-actions">
                {(activeLessonStep.hints || []).length > (lessonSession.hintsShown?.[activeLessonStep.id] || 0) && (
                  <button type="button" className="tb-btn" onClick={() => revealLessonHint(activeLessonStep.id)}>
                    Hint
                  </button>
                )}
                {(activeLessonStep.checks || []).some((check) => check.type === "manual") &&
                  !(lessonSession.completedStepIds || []).includes(activeLessonStep.id) && (
                    <button type="button" className="tb-btn primary" onClick={() => completeLessonStep(activeLessonStep.id, "manual")}>
                      Lock in answer
                    </button>
                  )}
                <button
          ...
          {lessonMobileTab === "tools" && (
            <div className="lesson-mobile-tools">
              <button
                type="button"
                className={[linkMode ? "active" : "", lessonNeedsCable ? "needs-attention" : ""].filter(Boolean).join(" ")}
              ...
              <button
                type="button"
                className={packetMode ? "active" : ""}
                aria-pressed={!!packetMode}
                onClick={() => setPacketMode((value) => (value ? null : { stage: "src" }))}
              >
                {Icon.packet()}
          ...
          {lessonMobileTab === "apps" && (
            <div className="lesson-mobile-apps">
              ...
              {openConsoles.map(
                (id) =>
                  devices[id] && (
                    <button type="button" key={id} onClick={() => setActiveBottom(id)}>
                      {devices[id].hostname} console
                    </button>
                  )
              )}
              {appTabsForWorkspace.map(
                (item) =>
                  devices[item.deviceId] && (
                    <button type="button" key={item.id} onClick={() => setActiveBottom(item.id)}>
                      {appBottomTabTitle(
                        devices[item.deviceId],
                        item.scope === "server" ? serverAppByKey(item.appKey) : endpointAppByKey(item.appKey)
                      )}
                    </button>
                  )
              )}
  ...
  const learnLessonWorkbench =
    learnLessonId && lessonSession?.lessonId === learnLessonId ? (
      <div
        ...
        <div className="learn-workbench-main">
          <div className="learn-workbench-canvas">
            <div className="learn-workbench-toolbar">
              <div className="learn-workbench-keys" aria-label="Lesson lab keys">
                <div className="learn-key-section learn-key-tools" aria-label="Mode tools">
                  <span>Tools</span>
                  <button
                    type="button"
                    className={[
                      "learn-tool-icon-button",
                      linkMode ? "active" : "",
                      lessonNeedsCable ? "needs-attention" : "",
                    ]
                      .filter(Boolean)
                      .join(" ")}
                    aria-label="Cable mode"
                    aria-pressed={!!linkMode}
                    title="Cable mode"
                  ...
                  <button
                    type="button"
                    className={["learn-tool-icon-button", packetMode ? "active" : ""].filter(Boolean).join(" ")}
                    aria-label="Packet mode"
                    aria-pressed={!!packetMode}
                    title="Packet mode"
                    onClick={() => setPacketMode(packetMode ? null : { stage: "src" })}
                  >
                    {Icon.packet()}
                  </button>
                </div>
                <div className="learn-key-section" aria-label="Topology status">
                  <span>Key</span>
                  <strong>{plural(Object.keys(devices).length, "device")}</strong>
                  <strong>{plural(links.length, "cable")}</strong>
                </div>
              </div>
              <div className="learn-workbench-step-pill">
                <span>{activeLessonStep?.kind || "Lesson"}</span>
                <strong>{activeLessonStep?.prompt || "Build and verify the topology."}</strong>
              </div>
            </div>
            <Topology
          ...
          <div className="bp-tabs">
            {openConsoles.map((id) => {
              ...
              return (
                <div key={id} className={`bp-tab device-tab ${isActive ? "active" : ""}`} onClick={() => setActiveBottom(id)}>
                  {Icon.terminal()}
                  <span style={{ textTransform: "none", letterSpacing: 0, fontFamily: "var(--font-mono)", fontSize: 12 }}>
                    {dev.hostname}
                  </span>
                  <span
                    className="close-tab"
            ...
            {[
              ...
              <div key={k} className={`bp-tab ${activeBottom === k ? "active" : ""}`} onClick={() => setActiveBottom(k)}>
                ...
                {badge != null && (
                  <span
                    className={`badge ${(k === "events" && events.some((e) => e.s === "err")) || (k === "validation" && validationErrorCount) ? "alert" : ""}`}
                  >
                    {badge}
                  </span>
                )}
          ...
          {!bottomCollapsed && (
            <div className="learn-workbench-panel">
              {openConsoles.map((id) => (
                <div key={id} style={{ position: "absolute", inset: 0, display: activeBottom === id ? "block" : "none" }}>
                  <CLI
                    ...
                    links={links}
                    onApply={(cmd) => {
                      markLessonAction(id, cmd);
                      onApplyToDevice(id, cmd);
                    }}
                    onPing={handlePing}
              ...
              {activeAppTab && activeAppDevice && activeEndpointApp && (
                <div style={{ position: "absolute", inset: 0, display: activeBottom === activeAppTab.id ? "block" : "none" }}>
                  <EndpointAppWorkspace
                    ...
                    onApplyCommand={(cmd) => {
                      markLessonAction(activeAppDevice.id, cmd);
                      onApplyToDevice(activeAppDevice.id, cmd);
                    }}
                    onPing={handlePing}
                    onTraceEvent={(trace) => recordPacketEvent(trace, simulatedDevices)}
                    ...
                    onHistoryChange={(history) =>
                      setCliHistory((m) => ({ ...(m && !Array.isArray(m) ? m : {}), [activeAppTab.id]: history }))
                    }
                    ghostSuggestions={cliGhostSuggestions}
                  />
                </div>
              ...
              {openConsoles.length === 0 &&
                !activeAppTab &&
                activeBottom !== "events" &&
                activeBottom !== "packets" &&
                activeBottom !== "validation" && (
              ...
              <div style={{ position: "absolute", inset: 0, display: activeBottom === "validation" ? "block" : "none" }}>
                <TopologyValidationPanel issues={validationIssues} devices={devices} links={links} onSelectIssue={selectValidationIssue} />
              </div>
            </div>
          )}
        </div>
    ...
    return (
      <>
        <OpenPTLearn.LessonPathView
          ...
          onResetAllProgress={resetAllLessonProgress}
          ...
          <AccountDialog
            ...
            onAccountDeleted={(deletion) => {
              ...
              resetSyncState({
                ...
                clearShare: true,
                clearSaveCounters: true,
                status: { state: "local", message: "Account deletion scheduled" },
              });
              setAccountOpen(false);
              setAccountInitial(null);
              ...
              onAction={(action) => {
                ...
                switch (action) {
                  ...
                  case "console":
                    ...
                    break;
                  case "show-int":
                    runConsoleCmd(id, "show interfaces");
                    break;
                  case "show-route":
                    runConsoleCmd(id, "show ip route");
                    break;
                  case "show-vlan":
                    runConsoleCmd(id, "show vlan brief");
                    break;
                  case "show-mac":
                    runConsoleCmd(id, "show mac address-table");
                    break;
                  case "show-run":
                    runConsoleCmd(id, "show running-config");
                    break;
                  case "power":
                    togglePower(id);
                    break;
  ...
  return (
    <div className="app" onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
      <input
      ...
      <div className="titlebar">
        ...
        <div className="tb-logo" onClick={() => navigateAppRoute("/")} style={{ cursor: "pointer" }} title="Back to home">
          <div className="glyph" />
          OpenPT
          ...
          <button
            ...
            onClick={(event) => {
              ...
              window.open(QUIZ_LIBRARY_URL, "_blank", "noopener,noreferrer");
            }}
          ...
          <button
            ...
            onClick={(event) => {
              event.stopPropagation();
              window.open(JEOPARDY_URL, "_blank", "noopener,noreferrer");
            }}
        ...
        <TitleMenus
          ...
          onReset={async () => {
            const ok = await requestConfirm({
              ...
              message: "Replace the current topology with the starter scenario?",
              confirmLabel: "Reset",
              danger: true,
            });
            if (!ok) return;
          ...
          onClearAll={async () => {
            const ok = await requestConfirm({
              ...
              message: "Remove every device and cable from the current tab?",
              confirmLabel: "Clear",
              danger: true,
            });
            if (!ok) return;
            ...
            setPacketEvents([]);
            setPtActivity(null);
            log("warn", "system", "topology cleared");
          }}
          ...
          onLab={async (key) => {
            if (key === "starter") {
              const ok = await requestConfirm({
                title: "Load starter lab?",
                message: "Replace the current topology with the starter lab?",
                confirmLabel: "Load lab",
                danger: true,
              });
              if (!ok) return;
          ...
          onLinkR1G01={() => {
            ...
            setDevices((m) => ({
              ...
              [r1.id]: {
                ...m[r1.id],
                interfaces: { ...m[r1.id].interfaces, [iface]: { ...m[r1.id].interfaces[iface], admUp: false, up: false } },
              },
            }));
            setLinks((ls) =>
              ls.map((l) => ((l.a === r1.id && l.ai === iface) || (l.b === r1.id && l.bi === iface) ? { ...l, up: false } : l))
            );
            log("warn", "R1", `${ifaceName(iface)} administratively shut down`);
          }}
          onEnterLinkMode={(type) => {
            setLinkMode(true);
            setForceLinkType(type);
          }}
        />
        ...
        <div className="tb-actions">
          {(cloudProjectId || shareToken) && meaningfulChanges > 0 && cloudLease && (
            <button
              className="tb-btn primary"
              onClick={() => saveCloudNow({ force: true }).catch((err) => setToast({ kind: "err", msg: err.message || "Save failed" }))}
            >
              Save Now
            </button>
          )}
          {(cloudProjectId || shareToken) && !cloudLease && shareMode !== "read" && (
            <button className="tb-btn primary" onClick={() => acquireCurrentLease(false)}>
              Edit
            </button>
          )}
          {cloudUser && !cloudProjectId && !shareToken && (
            <button className="tb-btn primary" onClick={createSyncedProject}>
              Save to cloud
            </button>
          )}
          <button className="tb-btn" onClick={() => setProjectsOpen(true)}>
            Projects
          </button>
          {cloudProjectId && !shareToken && (
            <button className="tb-btn" onClick={() => setShareOpen(true)}>
              Share
            </button>
          )}
          <button
      ...
      {readOnlyReason && (
        <div className="lease-banner">
          ...
          {shareMode !== "read" && (
            <button className="tb-btn" onClick={() => acquireCurrentLease(true)}>
              Take Over
            </button>
          )}
        </div>
      ...
      >
        ...
        {ptActivity && !ptSidebarOpen && (
          <div className="pt-sidebar-stub" onClick={() => setPtSidebarOpen(true)} title="Show assignment instructions">
            <span>▸</span>
          </div>
        )}
        ...
        <div className="center-col">
          <div className="tab-bar">
            ...
            <div className="tab-new" title="New blank tab" onClick={() => (isHomeRoute ? enterLabFromHome("lab") : newBlankTab())}>
              +
            </div>
            ...
            {!isHomeRoute && (
              <div className="tab-tools">
                <button
                  type="button"
                  className={`tab-tool ${linkMode ? "active" : ""}`}
                  title="Cable mode (L)"
                  aria-label="Cable mode"
                  aria-pressed={!!linkMode}
                  onClick={() => setLinkMode(!linkMode)}
                ...
                <button
                  type="button"
                  className={`tab-tool ${packetMode ? "active" : ""}`}
                  title="Packet mode (P)"
                  aria-label="Packet mode"
                  aria-pressed={!!packetMode}
                  onClick={() => setPacketMode(packetMode ? null : { stage: "src" })}
          ...
          {isHomeRoute ? (
            <div className="home-tab-surface">
              <iframe className="home-tab-frame" src="/home.html" title="OpenPT home" loading="eager" />
            </div>
          ) : (
            <Topology
              devices={simulatedDevices}
              ...
              {activeBottom === "events" && !bottomCollapsed && (
                <div className="event-tools">
                  {["all", "err", "warn", "ok"].map((kind) => (
                    <button key={kind} className={eventFilter === kind ? "active" : ""} onClick={() => setEventFilter(kind)}>
                      {kind}
                    </button>
                  ))}
                  <button
                    onClick={() =>
                      navigator.clipboard?.writeText(events.map((e) => `${e.t} ${e.s} ${e.src}: ${e.m}`).join("\n")).catch(() => {})
                    }
                  >
                    Copy
                  </button>
              ...
              {activeBottom === "packets" && !bottomCollapsed && (
                <div className="event-tools">
                  <button
                    onClick={() =>
                      navigator.clipboard
                        ?.writeText(packetEvents.map((e) => `${e.time} ${e.protocol} ${e.source}: ${e.summary}`).join("\n"))
                        .catch(() => {})
                    }
                  >
                    Copy
      ...
      {displayedImportReport &&
        (displayedImportReport.format === "packet-tracer-activity" ||
          displayedImportReport.unsupported ||
          displayedImportReport.reverseReport?.decoder ||
          displayedImportReport.diagnostics?.decoder) && (
          <ImportReportBanner
      ...
      {shareOpen && <ShareDialog onClose={() => setShareOpen(false)} onShare={createShareLink} shareUrl={lastShareUrl} />}
      ...
      <TweaksPanel>
        ...
        <TweakColor
          label="Accent"
          value={ACCENTS[t.accent]?.a || ACCENTS.cyan.a}
        ...
        <TweakSlider
          ...
          step={0.25}
          value={t.packetSpeed}
          onChange={(v) => setTweak("packetSpeed", v)}
          unit="×"
        />
        ...
        />
        <TweakButton
          label="Trigger PC1 → 8.8.8.8 (should fail)"
          onClick={() => {
            const pc1 = Object.values(devices).find((d) => d.hostname === "PC1");
...
function FeedbackWidget({ open, syncClient, onToggle, onClose }) {
  ...
  return (
    <div className={`feedback-overlay ${open ? "open" : ""}`}>
      {open && (
        <form className="feedback-card" onSubmit={submit}>
          <div className="feedback-head">
            <h2>Feedback</h2>
            <button type="button" className="feedback-close" onClick={onClose} aria-label="Close feedback">
              {Icon.close()}
            </button>
          </div>
          ...
          <div className="feedback-attachments">
            ...
            {attachments.map((attachment, index) => (
              <div key={`${attachment.filename}-${index}`} className="feedback-file">
                ...
                <button type="button" onClick={() => removeAttachment(index)} aria-label={`Remove ${attachment.filename}`}>
                  ×
                </button>
              </div>
            ))}
      ...
      <button type="button" className="feedback-button" onClick={onToggle}>
        Feedback
      </button>
    </div>
  );
...
function TitleMenus(props) {
  ...
  const menus = {
    File: [
      ...
      {
        label: "New starter scenario tab",
        on: () => {
          props.onNewStarterTab();
        },
      },
      {
        label: "Import lab file...",
        on: () => {
          props.onImportPacketTracer();
        },
      },
      { sep: true },
      { sect: "Export" },
      ...
      {
        label: "Export topology as JSON",
        on: () => {
          downloadJSON({ devices: props.devices, links: props.links }, "openpt-topology.json");
        },
      },
      {
        label: "Export selected device config",
        on: () => {
      ...
      {
        label: "Clear saved state & reload",
        on: () => {
          try {
            localStorage.removeItem("openpt:v1");
          } catch (e) {}
          location.reload();
        },
      },
            ...
            (menus[name].kind === "devices" ? (
                  ...
                  <div key={group.title}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, padding: "0 4px 6px" }}>
                      {group.devices.map((d) => (
                        ...
                        >
                          <div style={{ color: d.color }}>{React.createElement(Glyph[d.kind] || Glyph.router, { size: 28 })}</div>
                ...
                >
                  <span
                    style={{
                      ...
                      width: 18,
                      height: 2,
                      background: "repeating-linear-gradient(90deg, var(--magenta) 0 3px, transparent 3px 5px)",
                    }}
                  />
              ...
              <div className="tb-dropdown">
                {menus[name].map((it, i) => {
                  ...
                  if (it.sect)
                    return (
                      <div key={i} className="tb-dropdown-section">
                        {it.sect}
                      </div>
                    );
                  return (
        ...
        {user && mode === "account" ? (
          <>
            <div className="account-summary">
              ...
              <button className="tb-btn" disabled={submitting} onClick={onSignedOut}>
                Logout
              </button>
            </div>
            <div className="modal-actions">
              ...
              <button className="tb-btn" disabled={submitting} onClick={revokeOthers}>
                Sign out other sessions
              </button>
              <button className="tb-btn" disabled={submitting} onClick={() => loadSessions().catch(() => {})}>
                Refresh sessions
              </button>
            </div>
              ...
              {sessions.map((session) => (
                <div key={session.id} className={`session-row ${session.current ? "current" : ""}`}>
                  <div>
                    ...
                    <small>
                      {formatSessionTime(session.lastSeenAt)} · expires {formatSessionTime(session.expiresAt)}
                    </small>
                  </div>
                  <button className="tb-btn" disabled={submitting} onClick={() => revokeSession(session)}>
                    Revoke
                  </button>
                </div>
              ))}
            ...
            <form className="danger-zone" onSubmit={deleteAccount}>
              ...
              <button className="tb-btn" type="submit" disabled={submitting || !deletePassword}>
                Schedule deletion
              </button>
            </form>
          </>
          ...
          <form onSubmit={submit} className="account-form">
            ...
            {mode === "verify-sent" && <div className="sync-line">Check your inbox for the verification link.</div>}
            {mode === "reset-sent" && <div className="sync-line">Check your inbox for the password reset link.</div>}
            ...
            <div className="modal-actions">
              {mode === "verify-sent" ? (
                <>
                  <button className="tb-btn primary" type="button" disabled={submitting || !email} onClick={resendVerification}>
                    {submitting ? "Working..." : "Resend verification"}
                  </button>
                  <button
                    className="tb-btn"
                    type="button"
...
function ProjectsDialog({
  ...
  const localRows = (localProjects || []).map((project) => {
    ...
    const status = hasConflict
      ...
      : project.cloudProjectId
        ? dirtyTabs?.[project.id] || (isActive && syncStatus.state === "dirty")
          ? "Local changes"
          : "Synced copy"
        : "Local";
    return {
  ...
  const recentRows = [
    ...localRows,
    ...cloudRows.filter((row) => !localRows.some((local) => local.project.cloudProjectId === row.project.id)),
  ...
  const visibleRows =
    view === "local"
      ...
      : view === "cloud"
        ? cloudRows
        : view === "conflicts"
          ? recentRows.filter((row) => row.hasConflict)
          : recentRows;
  ...
  const openRow = (row) =>
    row.kind === "local" ? run(`open:${row.key}`, () => onOpenLocal(row.project)) : run(`open:${row.key}`, () => onOpen(row.project.id));
    ...
    <ModalShell
      title={
        <span className="project-modal-title">
          Projects <span>- {cloudUser?.email || "Local browser"}</span>
        </span>
      }
      onClose={onClose}
    >
      <div className="modal-body project-browser">
        ...
        <div className="project-list project-browser-list">
          ...
          {visibleRows.map((row) => (
            <div
              key={row.key}
              className={`project-row project-browser-row ${row.isActive ? "active" : ""} ${row.hasConflict ? "conflict" : ""}`}
            >
                ...
                {openMenu === row.key && (
                  <div className="project-row-menu">
                    {row.hasConflict && row.kind === "local" && row.cloud && (
                      <button type="button" disabled={!!pending} onClick={() => runMenu(`server:${row.key}`, () => onOpen(row.cloud.id))}>
                        Use server copy
                      </button>
                    )}
                    {row.kind === "local" && !row.project.cloudProjectId && (
                      <button
                        type="button"
                        disabled={!!pending || !cloudUser}
                        onClick={() => runMenu(`upload:${row.key}`, () => onUploadLocal(row.project))}
                      >
                        Upload to cloud
                      </button>
                    )}
                    ...
                    <button
                      type="button"
                      disabled={!!pending || editing === row.key}
                      onClick={() => {
                        setOpenMenu(null);
                        startRename(row);
                      }}
                    ...
                    <button
                      ...
                      onClick={() =>
                        runMenu(`dup:${row.key}`, () =>
                          row.kind === "local" ? onDuplicateLocal(row.project) : onDuplicateCloud(row.project)
                        )
                      }
                    >
                      Duplicate
                        ...
                        <div className="project-submenu-menu">
                          {["1m", "5m", "10m", "30m", "1h"].map((target) => (
                            <button
                              ...
                              type="button"
                              disabled={!!pending}
                              onClick={() => runMenu(`rollback:${target}`, () => onRollback(target))}
                            >
                              {target}
                            </button>
                          ))}
                    ...
                    <button
                      ...
                      disabled={!!pending}
                      onClick={() =>
                        runMenu(`del:${row.key}`, () => (row.kind === "local" ? onDeleteLocal(row.project) : onDeleteCloud(row.project)))
                      }
                    >
                      Delete
                    </button>
  ...
  return (
    <ModalShell title="Share project" onClose={onClose}>
      <div className="modal-body">
        <button className="tb-btn" disabled={!!pending} onClick={() => create("read")}>
          {pending === "read" ? "Creating..." : "Create read-only link"}
        </button>
        <button className="tb-btn primary" disabled={!!pending} onClick={() => create("edit")}>
          {pending === "edit" ? "Creating..." : "Create editable link"}
        </button>
        {shareUrl && (
          <div className="share-url">
            <span>{shareUrl}</span>
            <button className="tb-btn" onClick={() => navigator.clipboard?.writeText(shareUrl).catch(() => {})}>
              Copy
            </button>
          </div>
        )}
  ...
  return (
    <ModalShell title="Sync conflict" onClose={onClose}>
      <div className="modal-body">
        ...
        <button className="tb-btn" onClick={onLoadServer}>
          Load server copy
        </button>
        <button className="tb-btn" onClick={onDuplicate}>
          Keep local as duplicate
        </button>
        <button className="tb-btn primary" onClick={onTakeOver}>
          Take edit lease
        </button>
      </div>
    </ModalShell>
    ...
    <ModalShell title={title || "Confirm"} onClose={onCancel}>
      <div className="modal-body">
        ...
        <div className="modal-actions">
          ...
          <button className={`tb-btn ${danger ? "" : "primary"}`} onClick={onConfirm}>
            {confirmLabel}
          </button>
        </div>
      </div>
        ...
        {recentTargets?.length > 0 && (
          <div className="recent-row">
            {recentTargets.slice(0, 5).map((item) => (
              <button key={item} type="button" className="tb-btn" onClick={() => setTarget(item)}>
                {item}
              </button>
            ))}
          </div>
        )}
        <button className="tb-btn primary" type="submit" disabled={!valid}>
          Send ping
        </button>
      </form>
    </ModalShell>
...
function packetTracerIsConnectivityAssessment(item) {
  return /\b(connectivity|reachability|reachable|ping|icmp|trace\s*route|traceroute|simple\s+pdu|complex\s+pdu|pdu|successful\s+connection|packet\s+test)\b/i.test(
    packetTracerAssessmentText(item)
  );
}
...
function packetTracerAssessmentSections(activity) {
  ...
  if (!visibleItems.length && activity?.assessmentSections) {
    const storedItems = [...(activity.assessmentSections.assessmentItems || []), ...(activity.assessmentSections.connectivityTests || [])];
...
function packetTracerImportReport(activity) {
  ...
  if (devices.length)
    imported.push({ label: "Topology devices", detail: `${plural(devices.length, "device")} extracted from the logical workspace.` });
  if (links.length)
    imported.push({ label: "Topology links", detail: `${plural(links.length, "link")} extracted and connected on the canvas.` });
  if (assessmentCount)
    imported.push({
      label: "Assessment",
      detail: `${plural(assessmentCount, "assessment item")} imported, including ${sections.connectivityTests.length} connectivity checks.`,
    });
  else if (!activity.unsupported)
    skipped.push({ label: "Assessment", detail: "No visible scored assessment items were found or classified." });
  if (answerDeviceCount)
    imported.push({
      label: "Device configs",
      detail: `Running configuration lines were captured for ${plural(answerDeviceCount, "device")}.`,
    });
  if (decoded.xmlText || decoded.xmlLength)
    imported.push({
      label: "Decoded XML",
      detail: `Packet Tracer XML was preserved for follow-up analysis${decoded.xmlLength ? ` (${decoded.xmlLength} characters)` : ""}.`,
    });
  if (rawStorage.stored)
    imported.push({ label: "Original file", detail: `Raw PKA/PKT bytes were preserved in ${rawStorage.backend || "browser storage"}.` });
  else if (activity.rawFile)
    broken.push({ label: "Original file", detail: `Raw file preservation failed${rawStorage.reason ? `: ${rawStorage.reason}` : "."}` });

  if (hiddenObjects.length)
    skipped.push({
      label: "Hidden objects",
      detail: `${plural(hiddenObjects.length, "Packet Tracer object")} was not rendered on the OpenPT canvas.`,
    });
  ...
  if (activity.unsupported) {
    ...
    if (devices.length)
      approximated.push({ label: "Device models", detail: "Packet Tracer models were mapped to the closest OpenPT device/platform type." });
    if (links.length)
      approximated.push({ label: "Cable media", detail: "Packet Tracer cable details were normalized to OpenPT copper or serial links." });
    if (devices.length || links.length)
      approximated.push({ label: "Canvas layout", detail: "Logical workspace coordinates may be nudged to fit the OpenPT canvas." });
    if (assessmentCount)
      approximated.push({
        label: "Assessment checks",
        detail: "Packet Tracer rubric items were converted to OpenPT check rows; unsupported checks remain unchecked until implemented.",
      });
  }

  if (decoder.error && !activity.unsupported) broken.push({ label: "Decoder", detail: decoder.error });
  if (!activity.unsupported && !devices.length && !links.length)
    broken.push({ label: "Topology", detail: "The file decoded, but no renderable devices or links were extracted." });
...
function packetTracerGradeSourceItems(activity) {
  ...
  if (modelLeaves.length)
    return modelLeaves.map((item) => ({
      ...item,
...
function packetTracerIfaceByName(device, name) {
  ...
  return (
    Object.keys(device.interfaces || {}).find(
      (iface) => packetTracerIfaceKey(iface) === wanted || packetTracerIfaceKey(OPT_Engine.shortIfaceName?.(iface) || iface) === wanted
    ) || null
  );
}
...
function packetTracerItemTarget(item, devices) {
  ...
  if (item?.target?.deviceName || item?.target?.packetTracerDeviceName || item?.target?.saveRefId || item?.target?.memAddr) {
    device =
      packetTracerDeviceByName(devices, item.target.deviceName) ||
      packetTracerDeviceByName(devices, item.target.packetTracerDeviceName) ||
      Object.values(devices || {}).find((candidate) => {
        return [candidate.packetTracer?.saveRefId, candidate.packetTracer?.memAddr].some(
          (value) => value && (value === item.target.saveRefId || value === item.target.memAddr)
        );
      }) ||
      null;
  }
...
function packetTracerAnswerExpectations(activity, devices) {
  ...
  for (const [deviceName, commands] of Object.entries(activity?.answerCommands || {})) {
    ...
    for (const raw of commands || []) {
      ...
      if (match)
        setForIfaces((data) => {
          data.vlan = Number(match[1]);
          data.mode = data.mode || "access";
        });
      match = cmd.match(/^switchport trunk native vlan (\d+)$/);
      if (match)
        setForIfaces((data) => {
          data.nativeVlan = Number(match[1]);
          data.mode = "trunk";
        });
      match = cmd.match(/^switchport trunk allowed vlan (.+)$/);
      if (match)
        setForIfaces((data) => {
          data.allowedVlans = match[1];
          data.mode = "trunk";
        });
      match = cmd.match(/^switchport nonegotiate$/);
      if (match)
        setForIfaces((data) => {
          data.nonegotiate = true;
          data.mode = "trunk";
        });
      match = cmd.match(/^channel-protocol (lacp|pagp)$/i);
      if (match)
        setForIfaces((data) => {
          data.channelProtocol = match[1].toUpperCase();
        });
      match = cmd.match(/^channel-group (\d+) mode (\S+)$/);
      if (match)
        setForIfaces((data) => {
          data.channelGroup = { id: Number(match[1]), mode: match[2] };
        });
  ...
  return {
    ...
    feedback:
      ...
      (correct
        ? "Passed"
        : unchecked
          ? "OpenPT does not have a checker for this Packet Tracer item yet"
...
function packetTracerGradeTransferredState(item, base, context = {}) {
  ...
  return packetTracerGradeResult(
    item,
    { ...base, checkerId: "packet-tracer.transferred-state" },
    {
      status: "Correct",
...
function packetTracerAclSummary(device) {
  return Object.entries(device?.acls || {}).flatMap(([name, acl]) =>
    (acl.entries || []).map(
      (entry) => `${name} ${entry.action} ${entry.spec || [entry.proto, entry.src, entry.dst].filter(Boolean).join(" ")}`
    )
  );
}
...
const PACKET_TRACER_CHECKERS = [
  {
    ...
    grade: ({ device, expected }) => ({
      ...
      feedback: packetTracerCompare(device.hostname, expected.hostname)
        ? `${device.hostname} matches`
  ...
  },
  {
    ...
    grade: ({ text, device, iface, context }) => {
      ...
      if (/serial/.test(text))
        return {
          status: /serial/i.test(actual) ? "Correct" : "Incorrect",
          feedback: /serial/i.test(actual) ? "Serial link" : "Expected serial link",
          expected: "serial",
          actual,
        };
      if (/copper|ethernet|straight/.test(text) || /trunk configuration/.test(text))
        return {
          status: /serial/i.test(actual) ? "Incorrect" : "Correct",
          feedback: /serial/i.test(actual) ? "Expected Ethernet/copper link" : "Ethernet link",
          expected: "ethernet",
          actual,
        };
  ...
  },
  {
    ...
    grade: ({ text, device, iface, context }) => {
      ...
      if (!peerMatch)
        return {
          status: "Unchecked",
          feedback: "Expected link peer was not decoded for this item",
          evidence: { unsupportedReason: "missing-xml-mapping" },
        };
      const link = packetTracerFindLink(context.devices, context.links, device, iface);
      if (!link)
        return {
          status: "Incorrect",
          feedback: `No link on ${device.hostname} ${iface}`,
          expected: `${peerMatch[1]} ${peerMatch[2]}`,
          actual: null,
        };
      const { peer, peerIface } = packetTracerLinkPeer(context.devices, link, device);
      const peerNameOk =
        packetTracerCompare(peer?.hostname, peerMatch[1]) ||
        packetTracerCompare(peer?.name, peerMatch[1]) ||
        packetTracerCompare(peer?.packetTracer?.name, peerMatch[1]);
      ...
      return {
        ...
        feedback:
          peerNameOk && peerIfaceOk ? `Linked to ${peer.hostname} ${peerIface}` : `Expected link to ${peerMatch[1]} ${peerMatch[2]}`,
        expected: `${peerMatch[1]} ${peerMatch[2]}`,
  ...
  },
  {
    ...
    grade: ({ text, iface, expectedIface, actualIface }) => {
      ...
      if (!want)
        return {
          status: "Unchecked",
          feedback: "Expected switchport mode was not decoded",
          evidence: { unsupportedReason: "missing-xml-mapping" },
        };
      return {
        status: actualIface.mode === want ? "Correct" : "Incorrect",
        feedback: actualIface.mode === want ? `${iface} is ${want}` : `Expected ${iface} switchport mode ${want}`,
        expected: want,
        actual: actualIface.mode || "",
      };
    },
  },
  {
    id: "interface.vlan",
    supports: ({ text, actualIface }) =>
      /access vlan|vlan membership|assigned vlan|voice vlan|native vlan|allowed vlan/.test(text) && actualIface,
    grade: ({ text, item, iface, expectedIface, actualIface }) => {
      ...
      if (!want && !/allowed vlan/.test(text))
        return { status: "Unchecked", feedback: "Expected VLAN was not decoded", evidence: { unsupportedReason: "missing-xml-mapping" } };
      if (/voice vlan/.test(text))
        return {
          status: Number(actualIface.voiceVlan) === Number(want) ? "Correct" : "Incorrect",
          feedback: Number(actualIface.voiceVlan) === Number(want) ? `${iface} voice VLAN ${want}` : `Expected ${iface} voice VLAN ${want}`,
          expected: want,
          actual: actualIface.voiceVlan || "",
        };
      if (/native vlan/.test(text))
        return {
          status: Number(actualIface.nativeVlan) === Number(want) ? "Correct" : "Incorrect",
          feedback:
            Number(actualIface.nativeVlan) === Number(want) ? `${iface} native VLAN ${want}` : `Expected ${iface} native VLAN ${want}`,
          expected: want,
          actual: actualIface.nativeVlan || "",
        };
      if (/allowed vlan/.test(text)) {
        ...
        if (!allowed)
          return {
            status: "Unchecked",
            feedback: "Expected allowed VLAN list was not decoded",
            evidence: { unsupportedReason: "missing-xml-mapping" },
          };
        return {
          status: packetTracerCompare(actualIface.allowedVlans || "all", allowed) ? "Correct" : "Incorrect",
          feedback: packetTracerCompare(actualIface.allowedVlans || "all", allowed)
            ? `${iface} allowed VLANs ${allowed}`
            : `Expected ${iface} allowed VLANs ${allowed}`,
          expected: allowed,
          actual: actualIface.allowedVlans || "all",
        };
      }
      return {
        status: Number(actualIface.vlan) === Number(want) ? "Correct" : "Incorrect",
        feedback:
          Number(actualIface.vlan) === Number(want) ? `${iface} is assigned to VLAN ${want}` : `Expected ${iface} access VLAN ${want}`,
        expected: want,
        actual: actualIface.vlan || "",
      };
    },
  },
  {
    ...
    grade: ({ text, iface, expectedIface, actualIface }) => {
      if (/channel mode/.test(text)) {
        ...
        if (!want)
          return {
            status: "Unchecked",
            feedback: "Expected channel mode was not decoded",
            evidence: { unsupportedReason: "missing-xml-mapping" },
          };
        return {
          status: packetTracerCompare(actualIface.channelGroup?.mode, want) ? "Correct" : "Incorrect",
          feedback: packetTracerCompare(actualIface.channelGroup?.mode, want)
            ? `${iface} channel mode ${want}`
            : `Expected ${iface} channel mode ${want}`,
          expected: want,
          actual: actualIface.channelGroup?.mode || "",
        };
      }
      ...
      if (!want)
        return {
          status: "Unchecked",
          feedback: "Expected channel-group was not decoded",
          evidence: { unsupportedReason: "missing-xml-mapping" },
        };
      return {
        status: Number(actualIface.channelGroup?.id) === Number(want) ? "Correct" : "Incorrect",
        feedback:
          Number(actualIface.channelGroup?.id) === Number(want)
            ? `${iface} is in channel-group ${want}`
            : `Expected ${iface} in channel-group ${want}`,
        expected: want,
        actual: actualIface.channelGroup?.id || "",
      };
    },
  },
  {
    id: "interface.ip",
    supports: ({ text, actualIface }) =>
      /(ip address|subnet mask|default gateway|ospf priority|interface status|port status|shutdown)/.test(text) && actualIface,
    grade: ({ text, item, device, iface, actualIface }) => {
      ...
      if (/ip address/.test(text) && ip)
        return {
          status: packetTracerCompare(actualIface.ip, ip) ? "Correct" : "Incorrect",
          feedback: packetTracerCompare(actualIface.ip, ip) ? `${iface} IP matches` : `Expected ${iface} IP ${ip}`,
          expected: ip,
          actual: actualIface.ip || "",
        };
      if (/subnet mask/.test(text) && ip)
        return {
          status: packetTracerCompare(actualIface.mask, ip) ? "Correct" : "Incorrect",
          feedback: packetTracerCompare(actualIface.mask, ip) ? `${iface} mask matches` : `Expected ${iface} mask ${ip}`,
          expected: ip,
          actual: actualIface.mask || "",
        };
      if (/default gateway/.test(text) && device && ip) {
        const gateways = Object.values(device.interfaces || {})
          .map((ifc) => ifc.gw)
          .filter(Boolean);
        return {
          status: gateways.some((gw) => packetTracerCompare(gw, ip)) ? "Correct" : "Incorrect",
          feedback: gateways.some((gw) => packetTracerCompare(gw, ip)) ? "Default gateway matches" : `Expected default gateway ${ip}`,
          expected: ip,
          actual: gateways.join(", "),
        };
      }
      if (/ospf priority/.test(text)) {
        ...
        if (!want)
          return {
            status: "Unchecked",
            feedback: "Expected OSPF priority was not decoded",
            evidence: { unsupportedReason: "missing-xml-mapping" },
          };
        return {
          status: Number(actualIface.ospfPriority) === Number(want) ? "Correct" : "Incorrect",
          feedback:
            Number(actualIface.ospfPriority) === Number(want)
              ? `${iface} OSPF priority ${want}`
              : `Expected ${iface} OSPF priority ${want}`,
          expected: want,
          actual: actualIface.ospfPriority ?? "",
        };
      }
      if (/shutdown|interface status|port status/.test(text)) {
        ...
        return {
          status: Boolean(actualIface.admUp !== false && actualIface.up !== false) === wantUp ? "Correct" : "Incorrect",
          feedback: wantUp ? `Expected ${iface} up` : `Expected ${iface} shutdown`,
          expected: wantUp ? "up" : "down",
          actual: actualIface.admUp === false || actualIface.up === false ? "down" : "up",
        };
      }
      return {
        status: "Unchecked",
        feedback: "Expected interface value was not decoded",
        evidence: { unsupportedReason: "missing-xml-mapping" },
      };
    },
  },
  {
    ...
    supports: ({ text, device }) =>
      /(exec-timeout|transport input|ip ssh version|save config|startup|service password-encryption|domain-name|username|enable secret)/.test(
        text
      ) && device,
    grade: ({ text, item, device }) => {
      ...
      if (/exec-timeout/.test(text)) {
        ...
        if (!match)
          return {
            status: "Unchecked",
            feedback: "Expected exec-timeout was not decoded",
            evidence: { unsupportedReason: "missing-xml-mapping" },
          };
        ...
        return {
          ...
          feedback: ok ? `${line} exec-timeout ${match[1]} ${match[2]}` : `Expected ${line} exec-timeout ${match[1]} ${match[2]}`,
          expected: `${match[1]} ${match[2]}`,
          actual: actual ? `${actual.minutes} ${actual.seconds || 0}` : "",
        };
      }
      if (/transport input/.test(text)) {
        const want = String(value || item?.name || "")
          .match(/transport input\s+(.+)$/i)?.[1]
          ?.trim();
        if (!want)
          return {
            status: "Unchecked",
            feedback: "Expected VTY transport was not decoded",
            evidence: { unsupportedReason: "missing-xml-mapping" },
          };
        const actual = (device.lines?.vty?.transport || []).join(" ");
        return {
          status: packetTracerCompare(actual, want) ? "Correct" : "Incorrect",
          feedback: packetTracerCompare(actual, want) ? `vty transport input ${want}` : `Expected vty transport input ${want}`,
          expected: want,
          actual,
        };
      }
      if (/ip ssh version/.test(text)) {
        ...
        if (!want)
          return {
            status: "Unchecked",
            feedback: "Expected SSH version was not decoded",
            evidence: { unsupportedReason: "missing-xml-mapping" },
          };
        return {
          status: Number(device.ssh?.version) === Number(want) ? "Correct" : "Incorrect",
          feedback: Number(device.ssh?.version) === Number(want) ? `SSH version ${want}` : `Expected ip ssh version ${want}`,
          expected: want,
          actual: device.ssh?.version || "",
        };
      }
      if (/save config|startup/.test(text))
        return {
          status: device.startupConfig ? "Correct" : "Incorrect",
          feedback: device.startupConfig ? "Startup config saved" : "Expected startup-config to be saved",
          expected: "saved startup-config",
          actual: device.startupConfig ? "saved" : "empty",
        };
      const command = packetTracerCommandLikeExpected(item);
      if (!command)
        return {
          status: "Unchecked",
          feedback: "Expected service/line command was not decoded",
          evidence: { unsupportedReason: "missing-xml-mapping" },
        };
      const config = packetTracerConfigText(device);
      return {
        status: config.includes(packetTracerNormText(command)) ? "Correct" : "Incorrect",
        feedback: config.includes(packetTracerNormText(command)) ? "Expected config command found" : `Expected config command ${command}`,
        expected: command,
        actual: command && config.includes(packetTracerNormText(command)) ? command : "not present",
      };
    },
  },
  {
    ...
    supports: ({ text, device }) =>
      /(ip route|static route|router ospf|router rip|router eigrp|network |acl|access-list|nat|dhcp|spanning-tree|port-security|dhcp snooping|arp inspection)/.test(
        text
      ) && device,
    grade: ({ text, item, device }) => {
      const command = packetTracerCommandLikeExpected(item);
      ...
      if (command) {
        return {
          ...
          feedback: config.includes(packetTracerNormText(command)) ? "Expected config command found" : `Expected config command ${command}`,
          expected: command,
          actual: config.includes(packetTracerNormText(command)) ? command : "not present",
        };
      }
      if (/ip route|static route/.test(text)) {
        const routes = packetTracerRouteSummary(device);
        return routes.length
          ? { status: "Correct", feedback: "Static route present", expected: "static route", actual: routes.join("; ") }
          : { status: "Incorrect", feedback: "Expected static route", expected: "static route", actual: "none" };
      }
      if (/acl|access-list/.test(text)) {
        const acls = packetTracerAclSummary(device);
        return acls.length
          ? { status: "Correct", feedback: "ACL present", expected: "ACL entry", actual: acls.join("; ") }
          : { status: "Incorrect", feedback: "Expected ACL entry", expected: "ACL entry", actual: "none" };
      }
      if (/nat/.test(text)) {
        const rules = [...Object.keys(device.nat?.pools || {}), ...(device.nat?.rules || []).map((rule) => rule.config)].filter(Boolean);
        return rules.length
          ? { status: "Correct", feedback: "NAT configuration present", expected: "NAT config", actual: rules.join("; ") }
          : { status: "Incorrect", feedback: "Expected NAT configuration", expected: "NAT config", actual: "none" };
      }
      if (/dhcp/.test(text)) {
        const pools = Object.keys(device.dhcp?.pools || {});
        return pools.length || device.dhcpSnooping?.enabled
          ? {
              status: "Correct",
              feedback: "DHCP-related configuration present",
              expected: "DHCP config",
              actual: pools.join(", ") || "dhcp snooping",
            }
          : { status: "Incorrect", feedback: "Expected DHCP-related configuration", expected: "DHCP config", actual: "none" };
      }
      return {
        status: "Unchecked",
        feedback: "OpenPT simulator state does not yet expose this Packet Tracer check precisely",
        evidence: { unsupportedReason: "missing-simulator-capability" },
      };
    },
  },
  {
    id: "connectivity.plan-path",
    supports: ({ item, text, context }) =>
      packetTracerIsConnectivityAssessment(item) && typeof OPT_Engine.planPath === "function" && context.devices,
    grade: ({ text, item, context }) => {
      ...
      if (!src || !dstIp)
        return {
          status: "Unchecked",
          feedback: "Connectivity endpoints were not decoded for this item",
          evidence: { unsupportedReason: "missing-xml-mapping" },
        };
      ...
      return {
        ...
        feedback: result.ok
          ? `Reachable from ${src.hostname} to ${dstIp}`
          : result.error || `Not reachable from ${src.hostname} to ${dstIp}`,
        expected: `reachable ${dstIp}`,
        actual: result.ok ? "reachable" : "unreachable",
        evidence: { hops: result.hops || [] },
      };
    },
  },
  {
...
function packetTracerGradeItem(item, context) {
  ...
  if (transferred) return transferred;
  return packetTracerUnsupported(
    item,
    { ...base, checkerId: "unsupported" },
    "OpenPT does not have a checker for this Packet Tracer item yet"
  );
}
    ...
    <aside className="side-panel pt-side-panel">
      <div className="pt-side-title">
        <div>
          ...
          <div className="pt-side-name" title={title}>
            {title}
          </div>
        </div>
...
function PacketTracerInstructions({ activity }) {
  ...
  return (
    <div className="pt-empty">{activity?.instructionsText || "No assignment instructions were found in this Packet Tracer file."}</div>
  );
}
...
function packetTracerAssessmentPathParts(item) {
  ...
  return String(item?.path || item?.name || "Assessment Item")
    .split(/\s*\/\s*/)
    .filter(Boolean);
}
    ...
    <aside className="pt-check-summary">
      <div className="pt-check-score">
        ...
        <div>
          ...
          <span>
            {earnedItems}/{items.length}
          </span>
        </div>
        <div>
          <strong>Incorrect</strong>
          <span>{incorrectItems}</span>
        </div>
        <div>
          <strong>Unchecked</strong>
          <span>{uncheckedItems}</span>
        </div>
      </div>
      <div className="pt-component-table">
        <div className="pt-component-head">
          <span>Component</span>
        ...
        {components.map((component) => (
          <div className="pt-component-row" key={component.name}>
            <span title={component.name}>{component.name}</span>
            <span>
              {component.earnedItems || 0}/{component.items}
            </span>
            <span>
              {component.earnedPoints || 0}/{component.points}
            </span>
          </div>
        ))}
      </div>
...
function PacketTracerAssessmentRows({ items, empty }) {
  ...
  const unsupportedGroups = Object.entries(
    (items || [])
    ...
    <div className="pt-check-layout">
      <div className="pt-check-main">
        <div className="pt-check-actions">
          ...
          <button type="button" onClick={() => setExpanded(allExpanded ? new Set() : new Set(expandableKeys))}>
            Expand/Collapse All
          </button>
          <button type="button" onClick={() => setShowIncorrectOnly((value) => !value)}>
            {showIncorrectOnly ? "Show All Items" : "Show Incorrect Items"}
          </button>
            ...
            return (
              <React.Fragment key={row.key}>
                <div className="pt-check-row" role="row">
                  <span
                    className="pt-check-name"
                    style={{ paddingLeft: `${Math.max(0, row.depth) * 20 + 4}px` }}
                    title={item?.path || row.name}
                  >
                    {hasChildren ? (
                      <button
                        type="button"
                        className="pt-check-expander"
                  ...
                  <span title={item?.feedback || item?.attrs?.incorrectFeedback || item?.attrs?.feedback || ""}>
                    {item?.feedback || item?.attrs?.incorrectFeedback || item?.attrs?.feedback || ""}
                  </span>
                </div>
                {item && evidenceText && (
                  <div className="pt-check-row" role="row">
                    <span
                      className="pt-check-name"
                      style={{
                        paddingLeft: `${Math.max(0, row.depth) * 20 + 28}px`,
                        color: "var(--fg-3)",
                        gridColumn: "1 / -1",
                        fontFamily: "var(--font-mono)",
                        fontSize: 10.5,
                      }}
                      title={item.evidence?.decoded?.rawXml || ""}
                    >
                      {evidenceText}
                    </span>
                  </div>
  ...
  return (
    <aside className="server-module-sidebar" aria-label={`${device.hostname} server module`}>
      <div className="server-module-head">
        ...
        <button className="server-module-close" type="button" onClick={onClose} title="Close server module">
          ×
        </button>
      </div>
      <div className="server-module-tabs" role="tablist" aria-label="Server module tabs">
        <button type="button" className={activeTab === "config" ? "active" : ""} onClick={() => onTabChange("config")}>
          Config
        </button>
        <button type="button" className={activeTab === "desktop" ? "active" : ""} onClick={() => onTabChange("desktop")}>
          Desktop
        </button>
      </div>
      {activeTab === "desktop" ? (
    ...
    <div className="server-config-shell">
      <div className="server-config-nav" aria-label="Server Config sections">
        ...
        {SERVER_CONFIG_SECTIONS.map(([key, label]) => (
          <button key={key} type="button" className={activeConfig === key ? "active" : ""} onClick={() => onConfigChange(key)}>
            {label}
          </button>
        ))}
      </div>
      <div className="server-config-body">
        ...
        {activeConfig === "iot" && (
          <SimpleRegistryConfig title="Registration Server" section="iot" cfg={cfg.iot} onUpdate={onUpdate} serviceName="iot" />
        )}
...
function ServiceSwitch({ label = "Service", value, onChange }) {
  return (
    <div className="server-service-row">
      <span>{label}</span>
      <label>
        <input type="radio" checked={!!value} onChange={() => onChange(true)} /> On
      </label>
      <label>
        <input type="radio" checked={!value} onChange={() => onChange(false)} /> Off
      </label>
    </div>
  );
}
  ...
  return (
    <div className="server-table">
      ...
      <div className="server-table-body">
        ...
        {rows.map((row, i) => (
          <button
            key={row.id || row.name || row.username || i}
            type="button"
            className={`server-table-row ${selectedIndex === i ? "selected" : ""}`}
    ...
    <div className="server-section">
      ...
      <ServerTable
        columns={[
          ...
          { key: "edit", label: "Edit", width: "76px" },
          { key: "del", label: "Delete", width: "86px" },
        ]}
        rows={files.map((f) => ({ ...f, edit: f.editable ? "(edit)" : "", del: "(delete)" }))}
      ...
      <div className="server-actions">
        <button
          ...
          onClick={() => {
            ...
            if (!name) return;
            onUpdate((d, c) => {
              c.http.files.push({ name, editable: true, content: "" });
              d.files[`flash:${name}`] = "";
            }, `${name} created`);
          }}
        >
        ...
        <button
          ...
          onClick={() => {
            ...
            if (!name) return;
            onUpdate((d, c) => {
              c.http.files.push({ name, editable: /\.(html?|txt)$/i.test(name), content: "" });
              d.files[`flash:${name}`] = "";
            }, `${name} imported`);
          }}
        >
        ...
        <button type="button" disabled={selected == null} onClick={() => editFile(selected)}>
          Edit
        </button>
        <button
          type="button"
          ...
          onClick={() =>
            onUpdate((d, c) => {
              const [removed] = c.http.files.splice(selected, 1);
              if (removed) delete d.files[`flash:${removed.name}`];
            }, "HTTP file removed")
          }
        >
          Delete
  ...
  return (
    <div className="server-section">
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `DHCP service ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <div className="server-form-grid">
        <label>
          Pool Name
          <input value={form.poolName} onChange={(e) => setField("poolName", e.target.value)} />
        </label>
        <label>
          Default Gateway
          <input value={form.defaultGateway} onChange={(e) => setField("defaultGateway", e.target.value)} />
        </label>
        <label>
          DNS Server
          <input value={form.dnsServer} onChange={(e) => setField("dnsServer", e.target.value)} />
        </label>
        <label>
          Start IP Address
          <input value={form.startIp} onChange={(e) => setField("startIp", e.target.value)} />
        </label>
        <label>
          Subnet Mask
          <input value={form.subnetMask} onChange={(e) => setField("subnetMask", e.target.value)} />
        </label>
        <label>
          Maximum Number of Users
          <input value={form.maxUsers} onChange={(e) => setField("maxUsers", e.target.value)} />
        </label>
        <label>
          TFTP Server
          <input value={form.tftpServer} onChange={(e) => setField("tftpServer", e.target.value)} />
        </label>
        <label>
          WLC Address
          <input value={form.wlcAddress} onChange={(e) => setField("wlcAddress", e.target.value)} />
        </label>
      </div>
      <div className="server-actions">
        <button
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              ...
              if (idx >= 0) c.dhcp.pools[idx] = pool;
              else c.dhcp.pools.push(pool);
              c.dhcp.selectedPool = pool.poolName;
              d.dhcp.pools[pool.poolName] = {
                network: networkFromIpMask(pool.startIp, pool.subnetMask),
                mask: pool.subnetMask,
                defaultRouter: pool.defaultGateway,
                dnsServer: pool.dnsServer,
                startIp: pool.startIp,
                maxUsers: pool.maxUsers,
                tftpServer: pool.tftpServer,
                wlcAddress: pool.wlcAddress,
              };
            }, "DHCP pool saved")
          }
        >
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              c.dhcp.pools = c.dhcp.pools.filter((p) => p.poolName !== form.poolName);
              delete d.dhcp.pools[form.poolName];
              c.dhcp.selectedPool = c.dhcp.pools[0]?.poolName || "";
            }, "DHCP pool removed")
          }
        >
          Remove
      ...
      <ServerTable
        columns={[
          ...
          { key: "dnsServer", label: "DNS Server" },
          { key: "startIp", label: "Start IP Address" },
          { key: "subnetMask", label: "Subnet Mask" },
          { key: "maxUsers", label: "Max User", width: "72px" },
        ]}
        rows={cfg.pools || []}
        onSelect={(i) => setForm({ ...cfg.pools[i] })}
      />
    </div>
  );
}
...
function Dhcpv6Config({ cfg, onUpdate }) {
  ...
  useEffect(() => {
    ...
    if (selectedPool)
      setForm({
        poolName: selectedPool.poolName || "IPv6Pool",
        prefix: selectedPool.prefix || "2001:db8:1::",
        prefixLength: selectedPool.prefixLength || selectedPool.length || 64,
        gateway: selectedPool.gateway || "",
        dnsServer: selectedPool.dnsServer || "",
        domainName: selectedPool.domainName || "",
      });
  }, [cfg.selectedPool, cfg.pools?.length]);
  const setField = (key, value) => setForm((f) => ({ ...f, [key]: value }));
  return (
    <div className="server-section">
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `DHCPv6 service ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <div className="server-form-grid compact">
        ...
        <label>
          IPv6 Prefix
          <input value={form.prefix} onChange={(e) => setField("prefix", e.target.value)} placeholder="2001:db8:1::" />
        </label>
        <label>
          Prefix Length
          <input value={form.prefixLength} onChange={(e) => setField("prefixLength", e.target.value)} placeholder="64" />
        </label>
        <label>
          Default Gateway
          <input value={form.gateway} onChange={(e) => setField("gateway", e.target.value)} placeholder="fe80::1" />
        </label>
        <label>
          DNS Server
          <input value={form.dnsServer} onChange={(e) => setField("dnsServer", e.target.value)} />
        ...
        <label>
          Domain Name
          <input value={form.domainName} onChange={(e) => setField("domainName", e.target.value)} />
        </label>
      </div>
      <div className="server-actions">
        <button
          ...
          onClick={() => {
            setSelected(null);
            setForm({ poolName: "IPv6Pool", prefix: "2001:db8:1::", prefixLength: 64, gateway: "", dnsServer: "", domainName: "" });
          }}
        >
          Add
        </button>
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              ...
              if (idx >= 0) c.dhcpv6.pools[idx] = pool;
              else c.dhcpv6.pools.push(pool);
              c.dhcpv6.selectedPool = pool.poolName;
              d.dhcpv6.pools[pool.poolName] = {
                prefix: pool.prefix,
      ...
      <ServerTable
        columns={[
          { key: "prefix", label: "Prefix" },
          { key: "duid", label: "DUID" },
          { key: "pool", label: "Local Pool" },
          { key: "valid", label: "Valid Lifetime" },
        ]}
        rows={cfg.delegations || []}
      />
      ...
      <ServerTable
        columns={[
          { key: "poolName", label: "Pool Name" },
          { key: "prefix", label: "Prefix" },
          { key: "length", label: "Prefix Length" },
        ]}
        rows={cfg.localPools || []}
      />
      <h4>Bindings</h4>
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `TFTP service ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <ServerTable
        columns={[{ key: "name", label: "File" }]}
        rows={(cfg.files || []).map((name) => ({ name }))}
        selectedIndex={selected}
        onSelect={setSelected}
      />
      <div className="server-actions">
        <button
          type="button"
          onClick={() => {
            const name = window.prompt("TFTP file name");
            if (!name) return;
            onUpdate((d, c) => {
              c.tftp.files = [...new Set([...(c.tftp.files || []), name])];
              d.files[`flash:${name}`] = d.files[`flash:${name}`] || "";
            }, `${name} added`);
          }}
        >
          Add File
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              const [name] = c.tftp.files.splice(selected, 1);
              if (name) delete d.files[`flash:${name}`];
            }, "TFTP file removed")
          }
        >
          Remove File
  ...
  return (
    <div className="server-section">
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `DNS service ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <div className="server-form-grid compact">
        <label>
          Name
          <input value={record.name} onChange={(e) => setRecord({ ...record, name: e.target.value })} />
        </label>
        <label>
          Type
        ...
        <label className="span-2">
          Address
          <input value={record.detail} onChange={(e) => setRecord({ ...record, detail: e.target.value })} />
        </label>
      </div>
      <div className="server-actions">
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              c.dns.records.push({ ...record, no: c.dns.records.length + 1 });
            }, "DNS record added")
          }
        >
          Add
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              if (selected != null) c.dns.records[selected] = { ...record, no: selected + 1 };
            }, "DNS record saved")
          }
        >
          Save
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              if (selected != null) c.dns.records.splice(selected, 1);
            }, "DNS record removed")
          }
        >
          Remove
      ...
      <ServerTable
        ...
        onSelect={(i) => {
          setSelected(i);
          setRecord({ ...cfg.records[i] });
        }}
      />
      <button
        className="server-inline-button"
        type="button"
        onClick={() =>
          onUpdate((d, c) => {
            c.dns.cacheClearedAt = Date.now();
          }, "DNS cache cleared")
        }
      >
        DNS Cache
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `Syslog service ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <ServerTable
    ...
    <div className="server-section">
      ...
      <div className="server-service-row wide">
        ...
        <label>
          Radius Port
          <input
            value={cfg.radiusPort || "1645"}
            onChange={(e) =>
              onUpdate((d, c) => {
                c.aaa.radiusPort = e.target.value;
              }, "Radius port updated")
            }
          />
        </label>
      ...
      <div className="server-form-grid compact">
        ...
        <label>
          Client IP
          <input value={client.clientIp} onChange={(e) => setClient({ ...client, clientIp: e.target.value })} />
        </label>
        <label>
          Secret
          <input value={client.key} onChange={(e) => setClient({ ...client, key: e.target.value })} />
        </label>
        <label>
          ServerType
          <select value={client.serverType} onChange={(e) => setClient({ ...client, serverType: e.target.value })}>
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              ...
              c.aaa.users.push(user);
              d.users[user.username] = { secret: user.password };
            }, "AAA user added")
          }
        >
          Add
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `NTP service ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <fieldset className="server-fieldset">
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) =>
              onUpdate((d, c) => {
                c.ntp.password = e.target.value;
              }, "NTP password updated")
            }
          />
        </label>
      </fieldset>
    ...
    <div className="server-section">
        ...
        <ServiceSwitch
          ...
          onChange={(v) =>
            onUpdate(
              ...
              `POP3 ${v ? "enabled" : "disabled"}`
            )
          }
        />
      </div>
      <label className="server-wide-label">
        Domain Name:
        <input
          value={cfg.domain || ""}
          onChange={(e) =>
            onUpdate((d, c) => {
              c.email.domain = e.target.value;
            }, "Email domain set")
          }
        />
      </label>
      ...
      <div className="server-actions">
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              if (user.username) c.email.users.push(user);
            }, "Email user added")
          }
        >
          +
        ...
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              const target = c.email.users.find((u) => u.username === user.username);
...
function FtpConfig({ cfg, onUpdate }) {
  ...
  const togglePerm = (letter, checked) =>
    setUser((u) => {
  ...
  return (
    <div className="server-section">
      ...
      <div className="server-checkbox-row">
        {["W", "R", "D", "N", "L"].map((p) => (
          <label key={p}>
            <input type="checkbox" checked={user.permission.includes(p)} onChange={(e) => togglePerm(p, e.target.checked)} />{" "}
            {{ W: "Write", R: "Read", D: "Delete", N: "Rename", L: "List" }[p]}
          </label>
        ))}
      </div>
      <div className="server-actions">
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              if (user.username) c.ftp.users.push(user);
            }, "FTP user added")
          }
        >
          Add
      ...
      <div className="server-actions">
        <button
          ...
          onClick={() =>
            onUpdate((d, c) => {
              c.ftp.files.splice(selectedFile, 1);
            }, "FTP file removed")
          }
        >
          Remove
      ...
      <ServiceSwitch
        ...
        onChange={(v) =>
          onUpdate(
            ...
            `VM Management ${v ? "enabled" : "disabled"}`
          )
        }
      />
      <ServerTable
  ...
  return (
    <div className="server-section">
      ...
      <label className="server-check">
        <input
          type="checkbox"
          checked={!!cfg.allowEapMd5}
          onChange={(e) =>
            onUpdate(
              (d, c) => {
                c.radiusEap.allowEapMd5 = e.target.checked;
                d.services.radius = e.target.checked;
              },
              `EAP-MD5 ${e.target.checked ? "allowed" : "disabled"}`
            )
          }
        />{" "}
        Allow EAP-MD5
      ...
      <label className="server-check">
        <input
          ...
          onChange={(e) =>
            onUpdate(
              (d, c) => {
                c.prp.enabled = e.target.checked;
                d.services.prp = e.target.checked;
              },
              `PRP ${e.target.checked ? "enabled" : "disabled"}`
            )
          }
        />{" "}
        Enable PRP
...
function AppsSidebar({ device, activeAppKey, onOpenApp, onClose }) {
  const meta =
    DeviceCatalog.find((c) => c.platform === device.platform && c.kind === device.kind) ||
    DeviceCatalog.find((c) => c.kind === device.kind) ||
    DeviceCatalog[0];
  ...
  return (
    <aside className="apps-sidebar" aria-label={`${device.hostname} apps`}>
      <div className="apps-head">
        ...
        <button className="apps-close" type="button" onClick={onClose} title="Close apps">
          ×
        </button>
      </div>
    ...
    <div className="endpoint-app-page">
      <div className="endpoint-app-head">
        ...
        <div className="endpoint-app-title">
          <span>
            {device.hostname} - {app.label}
          </span>
          <small>{app.kind}</small>
        </div>
        <button type="button" className="endpoint-app-close" onClick={onClose} title="Close app">
          ×
        </button>
      </div>
      {isCliApp ? (
...
function ServerAppBody({ app, device, devices, links, onUpdateDevice, onRunSimulation, onPing }) {
  ...
  if (app.key === "mib")
    return <MibBrowserApp app={app} device={device} links={links} onUpdateDevice={onUpdateDevice} onRunSimulation={onRunSimulation} />;
  if (app.key === "vpn")
    return (
      <SessionConnectApp
        app={app}
  ...
  if (app.key === "communicator")
    return <CommunicatorApp app={app} device={device} links={links} onUpdateDevice={onUpdateDevice} onRunSimulation={onRunSimulation} />;
  if (app.key === "firewall" || app.key === "ipv6firewall")
    return <FirewallApp app={app} device={device} family={app.key} onUpdateDevice={onUpdateDevice} />;
  if (app.key === "iotmon" || app.key === "iotide")
    return (
      <IotApp
        app={app}
        device={device}
...
function ServerAccountingApp({ device }) {
  ...
  const mailboxes = Object.entries(device.appRuntime?.mail?.mailboxes || {}).map(([username, messages]) => ({
    username,
    ...
    <div className="endpoint-app-body">
      <div className="endpoint-table-panel">
        ...
        <div className="endpoint-table">
          ...
          {(cfg.aaa.users || []).map((user) => (
            <div key={user.username} className="endpoint-table-row">
      ...
      <div className="endpoint-table-panel">
        ...
        <div className="endpoint-table">
          ...
          {mailboxes.map((box) => (
            <div key={`mail-${box.username}`} className="endpoint-table-row">
              <span>Mailbox</span>
              <span>{box.username}</span>
              <span>{box.messages}</span>
            </div>
          ))}
          {registrations.map((reg) => (
            <div key={`voice-${reg.extension}`} className="endpoint-table-row">
              <span>Voice</span>
              <span>{reg.extension}</span>
              <span>Registered</span>
            </div>
          ))}
          {iot.map((reg) => (
            <div key={`iot-${reg.deviceId}`} className="endpoint-table-row">
              <span>IoT</span>
              <span>{reg.name}</span>
  ...
  if (app.key === "ip" || app.key === "wireless") {
    return (
      <EndpointIpApp
        ...
        device={device}
        devices={devices}
        preferWireless={app.key === "wireless"}
        onUpdateDevice={onUpdateDevice}
        onApplyCommand={onApplyCommand}
      />
    );
    ...
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel wide">
        ...
        <div className="endpoint-form-grid">
          ...
          <label>
            IPv4 Address
            <input value={ifc.ip || ""} onChange={(e) => updateIface("ip", e.target.value)} placeholder="192.168.1.10" />
          </label>
          <label>
            Subnet Mask
            <input value={ifc.mask || ""} onChange={(e) => updateIface("mask", e.target.value)} placeholder="255.255.255.0" />
          </label>
          <label>
            Default Gateway
            <input value={ifc.gw || ""} onChange={(e) => updateIface("gw", e.target.value)} placeholder="192.168.1.1" />
          </label>
          <label>
            DNS Server
            <input value={ifc.dns || ""} onChange={(e) => updateIface("dns", e.target.value)} placeholder="8.8.8.8" />
          </label>
          <label>
            IPv6 Assignment
            <select
              value={ifc.ipv6Source || (ifc.ipv6Autoconfig ? "slaac" : "static")}
              onChange={(e) => {
          ...
          <label>
            IPv6 Address
            <input value={ifc.ipv6 || ""} onChange={(e) => updateIface("ipv6", e.target.value)} placeholder="2001:db8:1::10" />
          </label>
          <label>
            IPv6 Prefix Length
            <input value={ifc.ipv6PrefixLength || ""} onChange={(e) => updateIface("ipv6PrefixLength", e.target.value)} placeholder="64" />
          </label>
          <label>
            IPv6 Gateway
            <input value={ifc.ipv6Gw || ""} onChange={(e) => updateIface("ipv6Gw", e.target.value)} placeholder="fe80::1" />
          </label>
          <label>
            IPv6 DNS Server
            <input value={ifc.ipv6Dns || ""} onChange={(e) => updateIface("ipv6Dns", e.target.value)} placeholder="2001:4860:4860::8888" />
          </label>
          {preferWireless && (
            <label>
              SSID
              <input value={ifc.ssid || ""} onChange={(e) => updateIface("ssid", e.target.value)} placeholder="OpenPT" />
            </label>
          )}
          {preferWireless && (
            <label>
          ...
          {preferWireless && wirelessSecurity === "wpa2-psk" && (
            <label>
              Passphrase
              <input value={ifc.passphrase || ""} onChange={(e) => updateIface("passphrase", e.target.value)} placeholder="openpt123" />
            </label>
          )}
        </div>
        {preferWireless && (
          <div className="endpoint-wireless-status">
            <div>
              <span>State</span>
              <strong>{ifc.associationState || "disconnected"}</strong>
            </div>
            <div>
              <span>AP</span>
              <strong>{associatedAp?.hostname || "-"}</strong>
            </div>
            <div>
              <span>Signal</span>
              <strong>{ifc.signalDbm != null ? `${ifc.signalDbm} dBm` : "-"}</strong>
            </div>
            <button type="button" onClick={connectWireless}>
              Connect
            </button>
            <button type="button" onClick={disconnectWireless}>
              Disconnect
            </button>
          </div>
        )}
        ...
        {rows.map((row) => (
          <div key={row.name} className="endpoint-table-row">
            ...
            <span title={`IPv4 ${row.ip} / ${row.mask}\nIPv6 ${row.ipv6}`}>
              {row.ip}
              {row.ipv6 !== "unassigned" ? ` | ${row.ipv6}` : ""}
            </span>
            <span title={`IPv4 ${row.gw}\nIPv6 ${row.ipv6Gw}`}>
              {row.gw}
              {row.ipv6Gw !== "unassigned" ? ` | ${row.ipv6Gw}` : ""}
            </span>
  ...
  return (
    <div className="endpoint-app-body single">
      <div className="endpoint-form-panel fill">
        ...
        <label className="endpoint-wide-label">
          File
          <input value={name} onChange={(e) => saveName(e.target.value || "desktop:notes.txt")} />
        </label>
        <textarea
        ...
        <div className={`endpoint-browser-page ${last?.ok === false ? "error" : ""}`}>
          ...
          {last ? (
            last.ok ? (
              <>
                <h3>
                  HTTP {last.status} {last.statusText}
                </h3>
    ...
    <div className="endpoint-app-body">
        ...
        <div className="endpoint-form-grid">
          <label>
            ...
            <input
              ...
              placeholder={`${device.hostname.toLowerCase()}@openpt.local`}
            />
          </label>
          <label>
            Incoming Server
            <input value={settings.pop3 || ""} onChange={(e) => set("pop3", e.target.value)} placeholder="192.168.20.20" />
          </label>
          <label>
            Outgoing Server
            <input value={settings.smtp || ""} onChange={(e) => set("smtp", e.target.value)} placeholder="192.168.20.20" />
          </label>
          <label>
            Password
            <input value={settings.password || ""} onChange={(e) => set("password", e.target.value)} placeholder="password" />
          </label>
        </div>
        <div className="server-actions">
      ...
      <div className="endpoint-form-panel">
        ...
        <div className="endpoint-form-grid">
          <label>
            To
            <input value={settings.to || ""} onChange={(e) => set("to", e.target.value)} />
          </label>
          <label>
            Subject
            <input value={settings.subject || ""} onChange={(e) => set("subject", e.target.value)} />
          </label>
        </div>
      ...
      <div className="endpoint-table-panel">
        ...
        <div className="endpoint-table">
          ...
          {inbox.map((msg) => (
            <div key={msg.id} className="endpoint-table-row">
              <span>{msg.from}</span>
              <span title={msg.body}>{msg.subject}</span>
              <span>{msg.time}</span>
            </div>
          ))}
...
function TrafficGeneratorApp({ app, device, links, onUpdateDevice, onRunSimulation, onPing }) {
  ...
  const run = () => {
    ...
    if (result?.ok && String(settings.protocol || "ICMP").toUpperCase() === "ICMP")
      onPing?.(device.id, result.targetIp || settings.destination);
  };
  return (
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel">
        ...
        <div className="endpoint-form-grid">
          <label>
            Destination
            <input value={settings.destination || ""} onChange={(e) => set("destination", e.target.value)} placeholder="192.168.20.20" />
          </label>
          <label>
            Protocol
            <select value={settings.protocol || "ICMP"} onChange={(e) => set("protocol", e.target.value)}>
              <option>ICMP</option>
              <option>TCP</option>
              <option>UDP</option>
              <option>HTTP</option>
          ...
          <label>
            Count
            <input value={settings.count || "5"} onChange={(e) => set("count", e.target.value)} />
          </label>
        </div>
        <div className="server-actions">
      ...
      <div className="endpoint-table-panel">
        ...
        <div className="endpoint-table">
          ...
          {history
            ...
            .reverse()
            .map((row, idx) => (
              <div key={`${row.time}-${idx}`} className="endpoint-table-row">
                <span>{row.time}</span>
                <span>{row.protocol}</span>
                <span title={row.result}>{row.ok ? "Delivered" : row.result}</span>
              </div>
            ))}
    ...
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel">
        ...
        <div className="endpoint-form-grid">
          <label>
            Agent
            <input value={settings.agent || ""} onChange={(e) => set("agent", e.target.value)} placeholder="192.168.1.1" />
          </label>
          <label>
            Community
            <input value={settings.community || "public"} onChange={(e) => set("community", e.target.value)} />
          </label>
          <label>
            OID
            <input value={settings.oid || "1.3.6.1.2.1.1.1.0"} onChange={(e) => set("oid", e.target.value)} />
          </label>
          <label>
            Set Value
            <input value={settings.value || ""} onChange={(e) => set("value", e.target.value)} />
          </label>
        </div>
        <div className="server-actions">
          <button type="button" onClick={() => query("get")}>
            Get
          </button>
          <button type="button" onClick={() => query("bulk")}>
            Get Bulk
          </button>
          <button type="button" onClick={() => query("set")}>
            Set
          </button>
        </div>
      </div>
      <div className="endpoint-status-panel">
        <AppLibraryIcon kind={app.key} />
        <div>
          <div className="endpoint-status-title">{last?.ok ? "SNMP Response" : "SNMP Status"}</div>
          <p>{last ? (last.ok ? last.value : last.error) : "No query sent"}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel">
        ...
        <div className="endpoint-form-grid">
          <label>
            Server
            <input value={settings.server || ""} onChange={(e) => set("server", e.target.value)} placeholder="192.168.20.20" />
          </label>
          <label>
            Username
          ...
          <label>
            Password
            <input value={settings.password || ""} onChange={(e) => set("password", e.target.value)} />
          </label>
        </div>
        <div className="server-actions">
      ...
      <div className="endpoint-status-panel">
        <AppLibraryIcon kind={app.key} />
        <div>
          <div className="endpoint-status-title">{session?.connected ? "Connected" : "Disconnected"}</div>
          <p>{session?.connected ? `${session.username} via ${session.server} at ${session.connectedAt}` : "No active session"}</p>
        </div>
      </div>
    </div>
  );
...
function CommunicatorApp({ app, device, links, onUpdateDevice, onRunSimulation }) {
  ...
  const register = () =>
    onRunSimulation?.((draft) => {
      const result = OPT_Services.registerCommunicator({
        devices: draft,
        links,
        sourceId: device.id,
    ...
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel">
        ...
        <div className="endpoint-form-grid">
          <label>
            Call Server
            <input value={settings.server || ""} onChange={(e) => set("server", e.target.value)} />
          </label>
          <label>
            Extension
            <input value={settings.extension || ""} onChange={(e) => set("extension", e.target.value)} />
          </label>
          <label>
            Password
            <input value={settings.password || ""} onChange={(e) => set("password", e.target.value)} />
          ...
          <label>
            Dial
            <input value={settings.dial || ""} onChange={(e) => set("dial", e.target.value)} />
          </label>
        </div>
        <div className="server-actions">
      ...
      <div className="endpoint-status-panel">
        <AppLibraryIcon kind={app.key} />
        <div>
          <div className="endpoint-status-title">{registration ? `Registered ${registration.extension}` : "Not registered"}</div>
          <p>{last || (registration ? `Server ${registration.serverId}` : "Register before placing calls")}</p>
        </div>
      </div>
    </div>
  );
...
function FirewallApp({ app, device, family, onUpdateDevice }) {
  ...
  return (
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel">
        <div className="endpoint-panel-title">{app.label}</div>
        <label className="endpoint-toggle-label">
          <input type="checkbox" checked={!!settings.enabled} onChange={(e) => saveSettings({ enabled: e.target.checked })} /> Firewall
          enabled
        </label>
        <div className="endpoint-form-grid">
          <label>
            Action
            <select value={draft.action} onChange={(e) => setDraft("action", e.target.value)}>
              <option>deny</option>
              <option>allow</option>
            </select>
          </label>
          <label>
            Protocol
            <select value={draft.protocol} onChange={(e) => setDraft("protocol", e.target.value)}>
              <option>tcp</option>
              <option>udp</option>
              <option>icmp</option>
              <option>any</option>
          ...
          <label>
            Source
            <input value={draft.src} onChange={(e) => setDraft("src", e.target.value)} />
          </label>
          <label>
            Destination
            <input value={draft.dst} onChange={(e) => setDraft("dst", e.target.value)} />
          </label>
          <label>
            Port
            <input value={draft.port} onChange={(e) => setDraft("port", e.target.value)} />
          </label>
          <label>
            Direction
        ...
        <div className="server-actions">
          ...
          <button type="button" onClick={() => saveSettings({ rules: [] })}>
            Clear
          </button>
        </div>
      </div>
      <div className="endpoint-table-panel">
        ...
        <div className="endpoint-table">
          ...
          {rules.map((rule) => (
            <div key={rule.id} className="endpoint-table-row">
              <span>{rule.action}</span>
              <span>
                {rule.protocol} {rule.src} to {rule.dst} port {rule.port}
              </span>
...
function IotApp({ app, device, links, onUpdateDevice, onRunSimulation, ide }) {
  ...
  const register = () =>
    onRunSimulation?.(
      (draft) =>
        OPT_Services.registerIotDevice({
          devices: draft,
          links,
          sourceId: device.id,
          server: settings.server || "",
          zone: settings.zone || "Lab",
        }),
      "IoT registered"
    );
  const run = () =>
    ...
    <div className="endpoint-app-body">
      <div className="endpoint-form-panel">
        ...
        <div className="endpoint-form-grid">
          ...
          <label>
            Zone
            <input value={settings.zone || "Lab"} onChange={(e) => set("zone", e.target.value)} />
          </label>
          {ide && (
            <label>
              Project
              <input value={settings.project || "main"} onChange={(e) => set("project", e.target.value)} />
            </label>
          )}
          {ide && (
            <label>
              Language
              <select value={settings.language || "JavaScript"} onChange={(e) => set("language", e.target.value)}>
        ...
        {ide && (
          <textarea
            className="endpoint-textarea compact"
            value={settings.code || ""}
            onChange={(e) => set("code", e.target.value)}
            placeholder="toggle('led0')"
          />
        )}
        <div className="server-actions">
          <button type="button" onClick={register}>
      ...
      <div className="endpoint-status-panel">
        <AppLibraryIcon kind={app.key} />
        <div>
          <div className="endpoint-status-title">{runtime.registration?.status || "Not registered"}</div>
          <p>{runtime.scripts?.slice(-1)[0]?.result || "No IoT activity"}</p>
        </div>
      </div>
    </div>
  );
          ...
          {spec.fields.map((field) => (
            <label key={field.key} className={field.type === "toggle" ? "endpoint-toggle-label" : ""}>
              {field.type === "toggle" ? (
                ...
                <>
                  {field.label}
                  <select value={settings[field.key] || field.default || ""} onChange={(e) => set(field.key, e.target.value)}>
...
function endpointUtilitySpec(key) {
  const specs = {
    dialup: {
      statusTitle: "Dial-up profile",
      statusText: "Stores modem dial details for lab workflows.",
      fields: [
    ...
    vpn: {
      statusTitle: "VPN profile",
      statusText: "Tracks tunnel parameters for remote access scenarios.",
      fields: [
    ...
    traffic: {
      statusTitle: "Traffic generator",
      statusText: "Use these values as the source profile for simulated traffic.",
      fields: [
    ...
    mib: {
      statusTitle: "SNMP query",
      statusText: "Stores SNMP browser query settings.",
      fields: [
    ...
    communicator: {
      statusTitle: "IP communicator",
      statusText: "Tracks softphone registration details.",
      fields: [
    ...
    pppoe: {
      statusTitle: "PPPoE session",
      statusText: "Stores broadband dialer credentials.",
      fields: [
    ...
    firewall: {
      statusTitle: "IPv4 firewall",
      statusText: "Endpoint firewall rules for IPv4 lab scenarios.",
      fields: [
    ...
    ipv6firewall: {
      statusTitle: "IPv6 firewall",
      statusText: "Endpoint firewall rules for IPv6 lab scenarios.",
      fields: [
        { key: "enabled", label: "Firewall enabled", type: "toggle" },
        { key: "defaultAction", label: "Default Action", type: "select", options: ["Allow", "Deny"], default: "Allow" },
    ...
    netflow: {
      statusTitle: "NetFlow collector",
      statusText: "Stores collector listener details.",
      fields: [
    ...
    iox: {
      statusTitle: "IoX workspace",
      statusText: "Tracks IoX project metadata for exercises.",
      fields: [
    ...
    tftp: {
      statusTitle: "TFTP service",
      statusText: "Stores file sharing settings for endpoint labs.",
      fields: [
    ...
    bluetooth: {
      statusTitle: "Bluetooth",
      statusText: "Stores local pairing state for wireless scenarios.",
      fields: [
    ...
    iotmon: {
      statusTitle: "IoT monitor",
      statusText: "Tracks endpoint IoT monitoring settings.",
      fields: [
    ...
    iotide: {
      statusTitle: "IoT IDE",
      statusText: "Stores the active IoT script profile.",
      fields: [
        { key: "project", label: "Project Name" },
  ...
  return (
    specs[key] || {
      statusTitle: "Application settings",
      statusText: "Stores local application state for this endpoint.",
      fields: [
      ...
      <div className="server-desktop-list" role="table" aria-label="Server desktop applications">
        ...
        {SERVER_DESKTOP_APPS.map((app) => (
          <button
            key={app.key}
            type="button"
            className={`server-desktop-row ${activeAppKey === app.key ? "active" : ""}`}
            role="row"
          ...
          >
            ...
            <span className="server-desktop-kind" role="cell">
              {app.kind}
            </span>
          </button>
        ))}
...
function ContextMenu({ x, y, device, onClose, onAction }) {
  ...
  const m =
    DeviceCatalog.find((c) => c.platform === device.platform && c.kind === device.kind) ||
    DeviceCatalog.find((c) => c.platform === device.platform) ||
    DeviceCatalog.find((c) => c.kind === device.kind) ||
    DeviceCatalog[0];
  // clamp to viewport
    ...
    <div className="events" ref={ref}>
      {events.length === 0 && (
        <div style={{ padding: 24, color: "var(--fg-3)", textAlign: "center" }}>
          No events yet. Run a ping or change a device to populate.
        </div>
      )}
      {events.map((e, i) => (
...
const PacketTracerReverseReport = React.memo(function PacketTracerReverseReport({ activity }) {
  ...
  const strings = (report.interestingStrings && report.interestingStrings.length ? report.interestingStrings : report.strings || []).slice(
    0,
    28
  );
  const entropyRows = (report.entropyByWindow || []).slice(0, 8);
  const download = () =>
    downloadJSON(activity, `${(activity.title || activity.sourceName || "packet-tracer").replace(/[^\w.-]+/g, "-")}-reverse-report.json`);
  ...
  return (
    <div className="events pt-check-window">
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: "var(--fg-0)", fontSize: 13, fontWeight: 600 }}>
            {activity.title || activity.sourceName || "Packet Tracer file"}
          </div>
          <div style={{ color: "var(--fg-2)", fontFamily: "var(--font-mono)", fontSize: 11, marginTop: 3 }}>
            {activity.unsupported ? "Reverse-engineering report" : "Extractor profile matched"} · {activity.sourceSize || report.size || 0}{" "}
            bytes
          </div>
        </div>
        {activity.rawFile?.storage?.stored && (
          <button className="hud-btn" style={{ width: "auto", padding: "0 10px", fontSize: 11 }} onClick={downloadRaw}>
            PKA
          </button>
        )}
        <button className="hud-btn" style={{ width: "auto", padding: "0 10px", fontSize: 11 }} onClick={download}>
          JSON
        </button>
      </div>
      ...
      {reportTab === "overview" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "130px 1fr", gap: "6px 12px", fontSize: 11.5, marginBottom: 14 }}>
            ...
            <code style={{ color: "var(--fg-1)", overflowWrap: "anywhere" }}>
              {activity.sourceSha256 || report.sha256 || "unavailable"}
            </code>
            ...
            <span style={{ color: activity.rawFile?.storage?.stored ? "var(--ok)" : "var(--warn)" }}>
              {activity.rawFile?.storage?.stored
                ? `preserved in ${activity.rawFile.storage.backend}`
                : `not preserved${activity.rawFile?.storage?.reason ? `: ${activity.rawFile.storage.reason}` : ""}`}
            </span>
              ...
              <>
                ...
                <span style={{ color: report.decoder.status === "decoded" ? "var(--ok)" : "var(--warn)", overflowWrap: "anywhere" }}>
                  ...
                  {report.decoder.profile
                    ? ` · ${report.decoder.profile}`
                    : report.decoder.attemptedProfile
                      ? ` · ${report.decoder.attemptedProfile}`
                      : ""}
            ...
            {activity.progress?.score && (
              <>
                ...
                <span style={{ color: "var(--ok)" }}>
                  {activity.progress.score} · {activity.progress.itemCount}
                </span>
              </>
            )}
          </div>
          ...
          {activity.unsupported && (
            <div style={{ color: "var(--warn)", marginBottom: 14, fontSize: 12 }}>
              No full extractor profile is packaged for this hash yet. The original file is preserved raw when browser storage is available,
              and unsupported Packet Tracer-only features are tracked below.
            </div>
          )}

          {assessmentCount > 0 && assessmentSections.connectivityTests.length === 0 && (
            <div style={{ color: "var(--warn)", marginBottom: 14, fontSize: 12 }}>
              Assessment data was decoded, but no connectivity tests matched the classifier. Check the assessment roots below to extend the
              import mapping for this PKA.
            </div>
          )}
      ...
      {reportTab === "connectivity" && (
        <div>
          <PacketTracerAssessmentRows
            items={assessmentSections.connectivityTests}
            empty="No connectivity tests were classified for this PKA."
          />
        </div>
      )}
      ...
      {reportTab === "raw" && (
        <>
          ...
          <div className="event" style={{ gridTemplateColumns: "130px 90px 1fr" }}>
            <span className="s dim">type</span>
            <span className="s dim">offset</span>
            <span className="m">signature</span>
          </div>
          {signatures.length === 0 ? (
...
function normalizeRubricPattern(pattern, assessmentItems) {
  ...
  if (Array.isArray(assessmentItems) && assessmentItems.length) {
    ...
    for (const item of assessmentItems) {
      const parts =
        Array.isArray(item.pathParts) && item.pathParts.length
          ? item.pathParts
  ...
  return (
    <div className="pt-sb-rub-node" style={{ paddingLeft: depth * 12 }}>
      <div className="pt-sb-rub-row" onClick={() => hasChildren && setOpen(!open)}>
        <span className="pt-sb-rub-toggle">{hasChildren ? (open ? "▾" : "▸") : "·"}</span>
        <span className="pt-sb-rub-label" title={label}>
          {label}
        </span>
        {node?.checkType && <span className="pt-sb-rub-type">{node.checkType}</span>}
        {points != null && points !== "" && <span className="pt-sb-rub-points">{points} pts</span>}
      </div>
      {hasChildren && open && (
...
function PacketTracerSidebar({ activity, onClose, onReportError, requestedTab, onRequestedTabHandled }) {
  const hasImportReport = !!(
    ...
    activity?.rawFile ||
    /\.(pka|pkt)$/i.test(activity?.sourceName || "")
  );
  ...
  return (
    <div className="pt-sidebar">
      <div className="pt-sb-head">
        <div className="pt-sb-title" title={title}>
          {title}
        </div>
        <div className="pt-sb-head-right">
          {hints.length > 0 && (
            <button
              className={`pt-sb-help ${topTab === "hints" ? "active" : ""}`}
              onClick={() => setTopTab(topTab === "hints" ? "instructions" : "hints")}
              title="Show lab hints"
            >
              ?
            </button>
          )}
          {hasImportIssue && onReportError && (
            <button className="tb-btn" style={{ padding: "3px 7px", fontSize: 11 }} onClick={onReportError}>
              Report Error
            </button>
          )}
          ...
          {onClose && (
            <button className="pt-sb-close" onClick={onClose} title="Hide sidebar">
              ×
            </button>
          )}
        </div>
      ...
      <div className="side-tabs pt-sb-tabs">
        {[
          ...
          <div key={k} className={`side-tab ${topTab === k ? "active" : ""}`} onClick={() => setTopTab(k)}>
            {lbl}
          </div>
        ))}
      </div>
      ...
      {topTab === "assessment" && (
        <div className="pt-sb-body">
          <div className="pt-sb-section">
            {assessmentOnly.length > 0 && <PacketTracerAssessmentRows items={assessmentOnly} empty="No assessment items extracted." />}
        ...
        <div className="pt-sb-body">
          <div className="pt-sb-section">
            <div className="pt-sb-summary">
              <div className="pt-sb-summary-row">
                <span className="k">Score</span>
                <span className="v">{progress?.score || "—"}</span>
              </div>
              <div className="pt-sb-summary-row">
                <span className="k">Items</span>
                <span className="v">{progress?.itemCount || `${allAssessmentItems.length}`}</span>
              </div>
              {typeof progress?.percent === "number" && (
                <div className="pt-sb-summary-row">
                  <span className="k">Percent</span>
                  <span className="v">{progress.percent}%</span>
                </div>
              )}
            </div>
            {components.length > 0 ? (
    ...
    <div className="packet-inspector">
      ...
      {events.length === 0 ? (
        <div className="packet-empty">
          No packet events yet. Run ping, DHCP, DNS, curl, SSH, FTP, SNMP, or packet mode to populate the inspector.
        </div>
      ) : (
        <div className="packet-inspector-body">
          <div className="packet-trace-list">
            ...
            {filtered.map((event) => (
              <button
                key={event.id}
                className={`packet-trace-row ${selected?.id === event.id ? "selected" : ""} ${event.status === "drop" ? "drop" : ""}`}
              ...
              <>
                <div className="packet-detail-head">
                  <div>
                    <div className="packet-title">
                      {selected.protocol.toUpperCase()} {selected.kind}
                    </div>
                    <div className="packet-subtitle">
                      {selected.source} {"->"} {selected.target || "unknown"}
                    </div>
                  </div>
                ...
                <div className="packet-section">
                  ...
                  <div className="packet-frame-columns">
                    <div>
                      <h5>L2</h5>
                      {renderFields(selected.frame?.l2)}
                    </div>
                    <div>
                      <h5>L3</h5>
                      {renderFields(selected.frame?.l3)}
                    </div>
                    <div>
                      <h5>L4/App</h5>
                      {renderFields({ ...(selected.frame?.l4 || {}), ...(selected.frame?.app || {}) })}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="packet-artifacts">
                    {(selected.artifacts?.aclHits || []).map((hit, index) => (
                      <div key={`acl-${index}`} className={`packet-artifact ${hit.action === "deny" ? "drop" : ""}`}>
                        ACL {hit.aclName} {hit.action}
                        {hit.sequence ? ` seq ${hit.sequence}` : ""} on {hit.device || hit.deviceId} {hit.iface || ""} {hit.direction || ""}
                        <span>{hit.spec}</span>
                      </div>
                    ))}
                    {(selected.artifacts?.natTranslations || []).map((nat, index) => (
                      <div key={`nat-${index}`} className="packet-artifact">
                        ...
                        <span>
                          {nat.outsideLocal || "-"} {"->"} {nat.outsideGlobal || "-"}
                        </span>
                      </div>
                    ))}
                    {selected.artifacts?.dhcpLease && (
                      <div className="packet-artifact">
                        DHCP lease {selected.artifacts.dhcpLease.ip}
                        <span>
                          router {selected.artifacts.dhcpLease.router || "-"} dns {selected.artifacts.dhcpLease.dns || "-"}
                        </span>
                      </div>
                    )}
                    {selected.artifacts?.dnsLookup && (
                      <div className={`packet-artifact ${selected.artifacts.dnsLookup.status === "nxdomain" ? "drop" : ""}`}>
                        DNS {selected.artifacts.dnsLookup.qname}
                        <span>{selected.artifacts.dnsLookup.answer || selected.artifacts.dnsLookup.status}</span>
                      </div>
                    )}
                    {selected.artifacts?.drop && (
                      <div className="packet-artifact drop">
                        Drop<span>{selected.artifacts.drop.reason || selected.summary}</span>
                      </div>
                    )}
                    {!selected.artifacts?.drop &&
                      !(selected.artifacts?.aclHits || []).length &&
                      !(selected.artifacts?.natTranslations || []).length &&
                      !selected.artifacts?.dhcpLease &&
                      !selected.artifacts?.dnsLookup && (
...
function PacketLog({ events }) {
  return (
    <div className="events">
      {events.length === 0 && (
        <div style={{ padding: 24, color: "var(--fg-3)", textAlign: "center" }}>
          No packets traced yet. Use the play button or run <code style={{ color: "var(--accent)" }}>ping</code> from a host CLI.
        </div>
      )}
      {events.map((e, i) => (
        <div key={i} className="event">
...
function PacketEventList({ events = [] }) {
  return (
    <div className="events">
      {events.length === 0 && (
        <div style={{ padding: 24, color: "var(--fg-3)", textAlign: "center" }}>
          No packet events yet. Run ping, traffic generator, or packet mode to capture protocol details.
        </div>
      )}
      {events
        .slice()
        ...
        .map((event) => (
          <div key={event.id || `${event.time}-${event.summary}`} className="event">
            ...
            <span className="m">
              {event.summary}
              {!!event.steps?.length && (
                <small style={{ display: "block", color: "var(--fg-3)", marginTop: 3 }}>
                  {event.steps
                    .map((step) => step.note || step.action)
                    .filter(Boolean)
                    .join(" · ")}
                </small>
              )}
...
function ImportReportBanner({ activity, onReport, onOpen, onClose }) {
  ...
  return (
    <div className={`import-report-banner ${hasBroken ? "has-broken" : report.hasIssues ? "has-warnings" : ""}`}>
      ...
      {hasBroken && onReport && (
        <button className="tb-btn" onClick={onReport}>
          Report Error
        </button>
      )}
      <button className="icon-btn" onClick={onClose}>
...
function FilesPanel({ devices, links }) {
  const fmt = JSON.stringify(
    {
      devices: Object.fromEntries(Object.entries(devices).map(([k, d]) => [k, { kind: d.kind, hostname: d.hostname }])),
      links: links.length,
    },
    null,
    2
  ...
  return (
    <div style={{ padding: "0 4px", fontFamily: "var(--font-mono)", fontSize: 11.5, color: "var(--fg-1)", overflow: "auto" }}>
      ...
      {[
        ...
        <div key={i} style={{ padding: "3px 12px", color: l.includes("●") ? "var(--accent)" : "var(--fg-1)" }}>
          {l}
        </div>
      ))}
    </div>
...
function LabsPanel({ onLoadStarter }) {
  return (
    <div style={{ padding: "0 8px", overflow: "auto" }}>
      ...
      {[
        {
          title: "Two-router VLAN routing",
          desc: "Configure inter-VLAN routing across two routers connected by a serial link.",
          active: true,
        },
        ...
        >
          <div
            style={{
              ...
              fontSize: 12,
              fontWeight: 500,
              color: l.active ? "var(--accent)" : "var(--fg-1)",
            }}
          >
            <span>{l.active ? "●" : "○"}</span>
            <span>{l.title}</span>
...
function TopologyValidationPanel({ issues = [], devices = {}, links = [], onSelectIssue }) {
  ...
  const visible = issues
    ...
    .sort(
      (a, b) =>
        severityOrder.indexOf(a.severity) - severityOrder.indexOf(b.severity) || String(a.category).localeCompare(String(b.category))
    );
  ...
  return (
    <div className="validation-panel">
      <div className="validation-summary">
        ...
        <button
          type="button"
          className={`validation-filter ${categoryFilter === "all" ? "active" : ""}`}
        ...
        {categories.map((key) => (
          <button
            key={key}
            type="button"
            className={`validation-filter ${categoryFilter === key ? "active" : ""}`}
      ...
      <div className="validation-list">
        ...
        {!!issues.length && !visible.length && (
          <div className="validation-empty subtle">No validation issues match the current filters.</div>
        )}
        {visible.map((issue) => {
              ...
              <button type="button" className="validation-issue-main" onClick={() => select(issue)}>
                ...
                <span className="validation-issue-text">
                  ...
                  <span className="validation-meta">
                    {categoryLabels[issue.category] || issue.category} · {targetLabel(issue)}
                  </span>
                </span>
                ...
                <div className="validation-detail">
                  ...
                  {!!issue.commands?.length && (
                    <div className="validation-commands">
                      ...
                      {issue.commands.map((cmd, index) => (
                        <code key={`${issue.id}-cmd-${index}`}>{cmd}</code>
                      ))}
                    </div>
                  )}
                </div>
...
function AnalysisPanel({ devices, links, events }) {
  ...
  return (
    <div style={{ padding: "0 8px", overflow: "auto" }}>
      ...
      {issues.length === 0 && <div style={{ padding: 10, color: "var(--ok)", fontSize: 11.5 }}>● All baseline checks passing.</div>}
      {issues.map((i, k) => (
      ...
      <div className="side-head" style={{ marginTop: 10 }}>
          ...
          v={
            new Set(
              Object.values(devices).flatMap((d) =>
                Object.values(d.interfaces)
                  .filter((i) => i.ip)
                  .map((i) => `${networkAddress(i.ip, i.mask)}/${OPT_Engine.maskBits(i.mask)}`)
              )
            ).size
          }
        />
...
function autoLinkType(a, b) {
  ...
  if ((OPT_Engine.isHostLike?.(a) || OPT_Engine.isHostLike?.(b)) && (OPT_Engine.isRouterLike?.(a) || OPT_Engine.isRouterLike?.(b)))
    return "cross";
  return "copper";
}
...
function parseAclEntry(action, spec, aclType) {
  ...
  const portValue = (value) =>
    ({ ftp: 21, ssh: 22, telnet: 23, dns: 53, tftp: 69, http: 80, https: 443 })[String(value || "").toLowerCase()] ||
    Number(value) ||
    value;
  const parsePort = (idx) => {
...
function networkAddress(ip, mask) {
  const ipI = OPT_Engine.ipToInt(ip),
    m = OPT_Engine.ipToInt(mask);
  const n = ipI & m;
